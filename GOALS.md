...
**SEO growth audit (D-056, 2026-08-02):** Search Console is verified and has been collecting data since 2026-07-26 — that part is done, correcting this section's prior "verify/submit... remains open" line, which was stale. What was actually open, and is the real explanation for near-zero organic numbers to date: the submitted sitemap had status "Couldn't fetch" in Search Console for the full 8 days since submission (0 discovered pages) despite the file itself being healthy — resubmitted this session. Also found: the live sitemap is stale relative to what's shipped (missing `/mlb/leaders`, generator supports it, just needs a re-run); `/mlb/game/{pk}` pages have no discovery path once they roll off the home page's daily snapshot; every edge-rendered page shares one static `og:image` regardless of content, undercutting the share-link-CTR metric D-041 named. Full findings in ISSUES.md D-056 — none of the fixes beyond the sitemap resubmit are implemented yet, pending a priority call.
