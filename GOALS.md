...
**NFL expansion push — kicked off 2026-08-02, owner-directed, three initiatives sequenced one at a time (not a single feature):** team roster grouped by individual position (**N-16, ✅ SHIPPED**, see ISSUES.md — replaces the 3 broad Offense/Defense/Special-Teams units with 9 position-specific groups). Next up, in order: an **injury report** surface (`injury_status` already flows through the Sleeper pool and already renders as a badge on roster rows and Top Assets cards — this would give it a dedicated page/view rather than just an inline tag) and a **waiver wire** surface (the existing NFL Trending view already covers Sleeper add/drop data — this is most likely an extension of that view rather than a from-scratch build, pending a Vera pass to confirm). Both scoped as two separate features, not a combined hub, per the owner's explicit call.
