...
**Fix (Axiom, same session):** added `_enrichMLBTeamAbbr(splits, season)` in `js/mlb.js` — factors the same id→abbreviation lookup `_fetchMLBLeaderSplits` already does into a reusable helper (fetches `AppState.mlbTeams` if not yet loaded, then backfills `team.abbreviation` from team id). Wired into both `mlbHotStats` population sites: the Leaderboards Hot Right Now strip (`js/mlb.js` ~4296) and `_renderHomeTrending()` (`js/app.js`). `_fetchMLBLeaderSplits`'s own working `enrich()` step is untouched. `node --check` clean, manifest/theme checks clean, 33/33 tests. `sw.js` bumped to v170. Pending: push + re-verify team abbreviations/colors render correctly live.
