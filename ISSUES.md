...
**Gate status: Vera's states spec + Kael's visual pass, both complete.** Needs Axiom's feasibility confirmation (cheapest path to season AVG/OPS for the hero without a new per-poll fetch, exact `battingOrder`-index math for Due Up, and the new `.lg-hero`/`.lg-dueup` markup fitting cleanly into `_renderPanel`'s existing render pass without a second diffing mechanism) before this is buildable. Once Axiom signs off, this is ready for implementation — live-verify against a real in-progress game before Phase 2 starts, per D-117.
