...
**Shipped — 2026-08-10 (Finn).** `css/main.css`: dead pre-D-046 hero block removed (`.home-hero-title`/`.home-hero-badge`/`.home-hero-glow`/`.home-hero-content`/`.home-hero-sub`/`.home-stats-strip`/`.home-stat`/`.home-stat-num`/`.home-stat-lbl` and its two 900px/600px orphaned overrides, plus one further orphaned `.home-stat-lbl` override found later in the file), `.home-hero`/`.hero-*` rescaled per the spec above, `.hero-live-detail` added. `js/app.js`: `_heroFromGame` now normalizes the live game via `Scorebug.normalizeMLBGame` and inserts `model.liveHtml` into the hero for live games. **Verified locally:** `node --check` clean, 0 NUL bytes on both changed files, `tools/check-manifest.cjs` clean, `tools/check-themes.cjs` clean (2 pre-existing unrelated warnings only), full 33-test unit suite passing, confirmed grep-clean of every dead class name across `css/`, `js/`, and `index.html`. Pending: `sw.js` bump, commit, push, live verification (watch for the same Cloudflare edge-cache propagation delay D-087 hit).

**Live-verified — 2026-08-10, after push.** Confirmed on production: `#scoreTicker` merges mlb/nfl/ncaaf items on Home, league glyphs render (⚾/🏈), live MLB game showed correctly (linescore state, team-color logos), reverts to sport-exclusive behavior outside Home. **One real deploy gotcha caught and resolved during verification, worth recording:** for several minutes after push, `js/scorebug.js` specifically (not `app.js`, not `ticker.css`) kept serving pre-deploy content even through a hard SW reinstall — `sw.js`'s install handler already does the right thing (`fetch(url, {cache:'reload'})` to bypass the *browser's* HTTP cache), but that only controls the browser layer; Cloudflare's *edge* cache for that exact asset path had its own independent TTL and hadn't been purged/revalidated yet, so even a cache-bypassing fetch from the SW kept hitting a stale edge copy (`cf-cache-status: REVALIDATED` against an old ETag) until the edge entry naturally expired (`cf-cache-status: EXPIRED` → fresh fetch). Resolved on its own within a few minutes; no action needed. **Diagnostic worth keeping for next time this happens:** a plain `fetch(url, {cache:'reload'})` from the browser only proves the *browser* isn't the stale layer — check `cf-cache-status` on the response to see whether Cloudflare's edge is still serving `HIT`/`REVALIDATED`-stale before concluding a deploy didn't take. A query-string-busted fetch (`?_=timestamp`) forces a different edge cache key and will show fresh content even while the plain URL is still stale — useful for confirming origin has the right file when the edge hasn't caught up yet.
