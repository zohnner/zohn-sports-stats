...
**Still open before production:** remote migration 0003 hasn't applied — `wrangler d1 migrations apply --remote` is currently failing with a Cloudflare API authorization error (`code: 7403`, account-token issue unrelated to this migration's content), same class of wrangler-auth flakiness seen earlier in this rollout. Must be resolved and the remote rate-limit table confirmed present before this ships live, or every real request will 500 in production exactly as it did locally. Also still pending: confirm `nodejs_compat` is set in the Cloudflare Pages dashboard (Production + Preview, not just local `wrangler.toml`); task #130 follow-star wiring; full `/security-review`.
