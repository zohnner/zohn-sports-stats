...
## Home page: `.sl-rail` sidebar overlaps/hides behind `.sl-primary` at mobile widths (≤960px) — found live on production, not yet fixed

**Contributor:** (session work, home redesign Phase 3) | **Date:** 2026-09-08, confirmed on live production

**Found while verifying an unrelated fix** (Phase 3's sport-picker snippet + its own `.home-sport-picker` grid-overflow fix, both in `css/main.css`). A CDP screenshot at a real mobile viewport (390px) showed the home page's sidebar content — sport-picker cards, the Following module (Phase 2), Recently Viewed — nowhere on the page between the hero/Today's-Games primary content and the bottom nav, even though `document.getElementById('homeSportPicker')` reported a real, non-zero-height, `display:grid` element. Comparing `.sl-primary`'s and `.sl-rail`'s `getBoundingClientRect()` directly: both report the identical `top` value (`281.1875px` in the reproduction) — the two are occupying the same grid row/space instead of stacking vertically once `.sl-layout`'s `@media (max-width: 960px)` rule collapses it from `grid-template-columns: 1fr 360px` to a single `1fr` column (`css/main.css:4796-4799`). Confirmed identical on **live production** at the same viewport (not a local-dev-only artifact): `sportstrata.cc/#home` shows the exact same overlapping `top` values.

**Practical impact:** on any viewport ≤960px (which includes essentially all phones and many tablets in portrait), the sport-picker, the Following module (followed teams/players, Phase 2), and Recently Viewed are effectively invisible — either painted behind `.sl-primary`'s taller content or otherwise not reachable by scrolling to where their bounding box claims to be. This has likely been true since Phase 0 (`cfd8e11`, 2026-09-07) introduced the `.sl-layout`/`.sl-primary`/`.sl-rail` shell for home, meaning it predates both Phase 2 and Phase 3 and was not caused by either — it was just never checked at a real mobile viewport with CDP's forced device-metrics override until this session's Phase 3 verification pass happened to do so.

**Root cause: not yet isolated.** Neither `.sl-primary` nor `.sl-rail` has an explicit `grid-row`/`grid-column`/`order` set anywhere in `css/main.css` (checked via `getComputedStyle` — both report `order: 0`), and plain CSS Grid auto-placement with a single `1fr` column track and two unplaced children should place them in separate auto-generated rows by default. Something is overriding that; candidates not yet checked: another selector elsewhere setting `grid-template-rows`/`grid-auto-flow` on `.sl-layout` or `.home-hub-layout` (the second class `loadHome()` adds to this same element, `js/app.js:680`, which currently has zero CSS rules of its own — confirmed via grep — so likely not the cause, but its existence with no rules is itself a little suspicious and worth a second look), or a specificity conflict from one of the *other* four `.sl-layout` usages (the sport-landing pages, `_renderEditorialLanding`) leaking a rule that home's `.home-hub-layout` variant doesn't actually want.

**Not fixed:** this is a bigger, riskier fix than Phase 3's own scope (a card's preview snippet) — the root cause is unconfirmed and the affected element is the home page's entire responsive shell, not one component. Needs its own investigation session: reproduce with CDP device-metrics override at 390px (do not trust `--window-size` alone — see this repo's own `/screenshot` skill notes on why), binary-search the CSS cascade for whatever's forcing both grid children onto the same row, fix, then re-verify all three sidebar modules (sport-picker, Following, Recently Viewed) are genuinely visible and stack correctly below the primary column at 390px, 640px, and 900px before calling it closed.
