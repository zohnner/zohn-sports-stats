...
### GOALS.md G6 "Sport Scope" conflicts with shipped NCAAF/NCAAB/WNBA reality — needs owner re-ratification — ✅ FIXED 2026-09-14
**Contributor:** Folio | **Date:** 2026-08-15 | **Resolved:** 2026-09-14, same pass as D-161 (NBA revival decision) — G6 rewritten to reflect the real 6-live-sport state and NBA's new active-revival status, old language kept visible for history rather than silently deleted.
...
**Fixed 2026-08-31 (D-121 follow-up), in a later session than the one that built the NCAAB/WNBA landing pages above — the original note's "bundle it in" suggestion got missed the first time, caught on a fresh SEO-audit pass instead.** `index.html`'s meta description, OG/Twitter description tags, and both JSON-LD `description` fields now name all five live sports (MLB, NFL, NCAA Football, NCAA Basketball, WNBA), plain factual phrasing per DESIGN.md's copy voice (no hype words). JSON-LD re-validated as parseable JSON after the edit.
...
**Idea 2 — ✅ SHIPPED 2026-09-13 (D-152), live-verified against real Week 1 plays.** **A real bug shipped with it, found and fixed same day (D-153):** the detection function read the live-situation data one level too shallow and silently never fired in the real browser until fixed — see D-153.

**Idea 1 — ✅ SHIPPED 2026-09-13 (D-154), same session as Idea 2 and the tension-layer follow-on.** Both ideas from this brainstorm are now built. See DECISIONS.md D-154.
...
**Kael — visual (against the owner's reference screenshot):**

**Intended feeling.** Right now the live page reads as *correct but polite* — every fact given equal weight is the same as no hierarchy at all. The reference image's whole argument is confidence: one glance should tell you the score, who's up, and that this page is watching the game *for* you in real time. That's the feeling to chase — not louder, not busier, just clearer about what matters most this second. Nothing here changes SportStrata's posture (broadcast-grade authority, dense-but-legible, zero premium friction) — this is a hierarchy and contrast pass, not a reskin, exactly as scoped.

**Hierarchy, resolved before any styling:** 1) score + who's up, 2) inning/count/live status, 3) Due Up + mini standings/leaderboard, 4) everything the sidebar already carries (game info, notes, umpires — pushed down, not removed). Today's page gives (1) and (2) the same weight and buries the closest thing to (1) — the batter — a click into the Matchup tab. That's the actual defect the reference image is reacting to, more than any single element being "too small."

**Header — score gets real dominance.** `.lg-score` moves from `--text-2xl` to `--text-4xl` (already in the token set — `clamp(2rem, 5vw, 3rem)`, so this scales itself on mobile without a media-query rule), weight stays 800, font stays `--font-display` with `tabular-nums`. `.lg-abbr` steps up one size (`--text-sm` -> `--text-lg`) so the team identity doesn't look orphaned next to a much bigger number — a hierarchy move needs its whole cluster to move together or the relationship reads as broken, not intentional. `.lg-meta-row` (inning + count + LIVE badge) collapses from three same-weight chips into one line: inning/half + count as a single mono phrase at `--text-sm` in `--text-secondary` (not `--text-muted` — this is state, not decoration, per DESIGN.md's three-level text discipline), with the LIVE badge (unchanged — `--color-live` dot + pulse, already correct per D-047) sitting at the end of that same line rather than centered as its own row. This is the actual fix for the "contrast is inconsistent" complaint in the analysis: apply the site's own existing three-level text system (`--text-primary` / `--text-secondary` / `--text-muted`) consistently here, rather than the current mix where inning and count read at nearly identical weight to each other. No new tokens — DESIGN.md already has this vocabulary, this page just wasn't using all three rungs of it.

**New: batter/pitcher hero block (`.lg-hero`).** Sits between the linescore and the tab row — new territory, not a retrofit of an existing card. Two-column card, same shell language as `.lg-side-card` (`--bg-surface`, `--border-default`, `--radius-md`) so it doesn't invent a new surface language, but full-width and taller: this is the primary content of the page, not a sidebar fact. Left half = batter, right half = pitcher, divided by a single `--border-default` rule (not a heavier team-color divider — team identity already lives in the panel's left edge per Border = Identity; duplicating it here would be noise). Each side: name in `--font-sans` 600 weight `--text-lg` `--text-primary` (this is a person's name, not a stat — Inter, not Barlow), one stat line directly beneath in `--font-mono` `--text-secondary` (season AVG/OPS for the batter, today's pitch count + last pitch velocity for the pitcher — the Receipts pattern: never show a number without saying what it is, so this renders as `.278 AVG` / `92.4 MPH`, labeled, not bare). No photography — this project has no licensed headshot pipeline for a hero this prominent (same constraint Trophy Case hit and solved by not using photography at all); a small circular initial-badge in the batting/throwing team's color (reusing the same team-color-as-inline-style convention already sanctioned for logos) stands in, not a placeholder box.

**Hero motion, deliberately restrained.** The hero content re-renders on every poll like the rest of the page, but it does **not** replay an entrance animation on a poll that doesn't actually change the batter or pitcher — only a genuine matchup change (new batter announced, pitching change) gets the existing `lgEntrySlide`-class treatment (200ms ease-out, already defined, already respects `prefers-reduced-motion`). Motion marks a real state change, never a refresh — this is DESIGN.md's motion rule applied directly, and it's also what stops the hero from feeling twitchy on a 9-second poll cadence.

**Due Up rail (`.lg-dueup`).** Not a new card — a compact strip beneath the hero, three small entries (next batters), each just a name + position abbreviation in the existing `.lg-side-row`/`.lg-side-val` idiom already used sitewide for this exact kind of compact fact-list. No photography here either — names carry this, at this size a headshot would compete with the hero for attention it hasn't earned.

**Mini standings + mini leaderboard — sidebar, not hero.** These are context, not the thing the page is about right now — they belong in `.lg-sidebar` alongside Game Info, ahead of Umpires/Game Notes in document order (Vera's priority reordering) but styled identically to the existing `.lg-side-card` pattern, not elevated further. Mini standings: team abbreviation + team color chip (existing convention) + W-L-GB in `--font-mono` `tabular-nums`, 5 rows max (the home team's full division), no new table component — this is `.lg-side-row` repeated. Mini leaderboard: 3-5 rows, stat value in `--font-mono`, player name in `--font-sans`, using the site's existing stat-category color only if the category already has one assigned in `MLB_LEADER_CATS` (never inventing a new hue for this — Category Color Discipline is explicit that stat colors mark category, not importance, and a mini-leaderboard entry isn't more important than the same stat elsewhere on the site).

**Broadcast chip.** One line inside the existing Game Info `.lg-side-card`, `--text-xs` `--text-muted` — this is the lowest-priority fact on the whole page, styled to read that way. If Relay's hydrate check comes back empty, this line is absent, not blank space held open for it (no skeleton needed either — Vera's spec already says this one doesn't get a placeholder).

**Color: zero new tokens for this phase.** Everything above reuses `--text-primary/secondary/muted/subtle`, `--color-live`, `--bg-surface`, `--border-default`, existing stat-category hues, and team colors via the already-sanctioned inline-style path. `--accent` appears nowhere new in this phase — brand orange stays reserved for nav/CTA per D-047 invariant #3, and nothing in this restructure is a primary action.

**Stress test.** Long names (`Ronald Acuña Jr.`, `Giancarlo Stanton`) at `--text-lg` in the hero: allow wrap to a second line rather than truncating — a truncated player name reads as a bug, not density. Due Up at a lineup-wrap boundary (9th batter due up is 1st in the order again): no special case, `battingOrder` is already circular. Two teams sharing a near-identical team color (e.g. both wearing navy): border-as-identity already handles this sitewide via each team's own hex, not a semantic slot, so no new collision risk introduced here. 320px mobile viewport: hero's two columns stack full-width (batter above pitcher — matches Vera's spec), Due Up stays a single row of three compact items rather than wrapping to a second visual row, which would push it too close to the hero's own weight.

**What's explicitly not designed in this pass:** the pitch-mix wheel and Bullpen Availability panel from the reference image — those are D-117 Phases 2/3, and get their own Kael pass when Relay's `breakHorizontal` check and the bullpen-integration task are ready, not designed ahead of their gate.

---

**Axiom — feasibility:**

**Season AVG/OPS for the batter half of the hero — one new fetch, gated on batter change, not on poll.** `feed.gameData.players['ID'+id]` is bio-only (confirmed by reading `_buildProbablePitchers`'s own comment: `gameData.probablePitchers` "only carries id/fullName/link" — same shape problem, same file). Season stats need `mlbFetch('/people/{batterId}/stats', {stats:'season', group:'hitting', season: MLB_SEASON}, ApiCache.TTL.MEDIUM)` — exact mirror of the already-shipped `_lgFetchH2H` pattern (`liveGame.js` ~981), same cache-by-key discipline, new key is just `playerId` (the season is implicit — `MLB_SEASON` doesn't change mid-session, so no need to fold it into the cache key). New module state: `_lgSeasonStatCache = {}`. This fires once per real batter change, not once per 9s poll: `_renderPanel` renders the hero shell synchronously from `matchup.batter.fullName` (already have it, no fetch), then kicks off the cached fetch in the background and patches just the stat-line node when it resolves — same fire-and-forget-then-patch shape already proven for the pitching-change banner (`_lgLastPitcherId` diff at `_doPoll` line ~213). Guard against a race where the batter changes again before the fetch resolves: store `data-batter-id` on the hero's batter-side node, check it still matches before writing the patched stat line, plus the existing `_lgFeedCache === feed` guard already used for the Matchup tab's own async patch (line ~1096). No architectural risk here — this is the same pattern the file already runs twice.

**Pitcher's pitch count + last velocity — zero new fetch.** Both already flow through data `_renderPanel` has in hand every poll: `numberOfPitches` is already read for the box score pitching table (`_buildBoxScore`, from `boxscore.teams[side].players[pid].stats.pitching.numberOfPitches`); last-pitch velocity is `p.startSpeed` on the most recent `isPitch` playEvent of `currentPlay` — the exact field the pitch-zone dots already render. Confirms Kael's "cheapest path" framing was right for the pitcher half without qualification.

**Due Up — confirmed pure computation, zero new fetch.** Offense side is `ls.isTopInning ? 'away' : 'home'`. `boxscore.teams[offenseSide].battingOrder` is the same array `_buildBoxScore` already reads. Current batter id from `currentPlay.matchup.batter.id`; index via `order.indexOf(batterId)`; next three via `order[(idx+n) % order.length]`, which is safe even on a short/irregular order array as long as it's non-empty. If `indexOf` returns -1 (data gap — shouldn't happen in practice but not provably impossible), the rail renders nothing rather than guessing, matching Vera's spec and this file's existing fail-closed convention elsewhere (e.g. `_buildSidebar`'s empty-state fallback).

**`.lg-hero`/`.lg-dueup` markup fits the existing render pass with no new diffing mechanism.** `_renderPanel` (liveGame.js:323) already works by targeting specific child elements and replacing their `innerHTML` wholesale each poll — `.lg-header`, `.lg-linescore-wrap`, the tab content, the sidebar are each just one `querySelector(...).innerHTML = build...()` call. Two new containers (`.lg-hero`, `.lg-dueup`) get added to `_buildSkeletonPanel`'s initial template between the poll-timestamp row and `.lg-body`, and two new lines in `_renderPanel` follow the identical pattern. No new mechanism, no risk to the existing render architecture.

**Restrained hero motion (Kael's "no animation on an unchanged matchup") — one new comparison, same shape as the existing pitching-change detector.** New module state `_lgLastHeroBatterId`, compared against `matchup.batter?.id` on each render; only when it changed does the freshly-built hero HTML carry the existing `lgEntrySlide`-class treatment (already defined, already respects `prefers-reduced-motion`). No new animation code — reusing what's there.

**Real gap Kael's/Relay's specs both assumed away — flagging before this ships, not after.** Mini standings and mini leaderboard were specced as "reuse existing app-wide state" (`AppState.mlbStandings`, `MLB_LEADER_CATS`/leader splits). That's true *if* the user reached this page by clicking through from Standings or the home page — it is **not** guaranteed true for a cold direct link to `mlb-live-{id}` (a shared URL, a bookmark, a fresh tab), which is a realistic and probably common entry point for exactly this page, not an edge case. `AppState.mlbStandings` is lazily populated only by `loadMLBStandings()` (per CLAUDE.md's own MLB views table) and I have not confirmed `AppState.mlbLeaderSplits` is warm on a cold boot into this route either. **Resolution:** `showMLBLiveGame` gets a cheap guard — if `AppState.mlbStandings` (or the leader splits it needs) isn't already populated, fetch just what this widget needs (the two teams' division standings; the site's existing small leader category set) via the already-existing `mlbFetch` calls those features use, cached `LONG`/`MEDIUM` per the established TTL table — not the full standings/leaderboard payload sitewide. This is a small, contained addition, not new architecture, but it wasn't in either upstream spec and needs to be in Phase 1's scope, not discovered live.

**Broadcast chip — deferred out of Phase 1, not built against an unconfirmed field.** Relay flagged the MLB schedule `broadcasts` hydrate as "likely buildable, unverified." Per this project's own standing rule (don't build on an unconfirmed field), this one line ships in a small follow-up once Relay confirms the hydrate is populated for MLB — Phase 1 ships without it. Kael's spec already allows for this element to be absent without a placeholder, so nothing else in the design needs to change.

**No script-load-order change, no new external dependency, no CSP change.** Everything above stays inside `liveGame.js`/`liveGame.css`, using `mlbFetch`/`ApiCache` exactly as they already work.

**Sign-off:** feasible as specced, with the standings/leaderboard cold-load gap folded into this phase's scope and the broadcast chip deferred to its own follow-up. Ready for implementation.
...
## MLB live game viewer: Batting/Pitching hero card and Due Up rail showed the wrong team between half-innings — live debug + fix, 2026-08-31

**Contributor:** Axiom | **Date:** 2026-08-31, live-verified against a real in-progress game (SF @ ATL, gamePk 824911)

**Found while live-debugging the shipped viewer against a real game, not a reported complaint.** Cross-checked the rendered panel against `statsapi.mlb.com`'s own feed directly (not just eyeballed) and caught the panel showing "BATTING: Christian Koss" — a San Francisco batter who had just made the third and final out of the top 2nd — as the live current batter, with "DUE UP" also still listing San Francisco's own next hitters, while the real game had already moved to `linescore.inningState: "Middle"` with Atlanta's Michael Harris II correctly up per MLB's own feed.

**Root cause:** `_buildHero`, `_lgMaybeFetchHeroBatterLine`, and `_buildDueUp` (all `js/liveGame.js`) all sourced the current batter/pitcher from `feed.liveData.plays.currentPlay.matchup`, plus a raw `linescore.isTopInning` check for team side. `currentPlay.matchup` only advances once the new half's first pitch is actually thrown — for the whole Middle/End window between half-innings (anywhere from ~30 seconds to a couple minutes), it still points at the batter and pitcher from the just-finished at-bat. The header's ball-strike-out count pill already had a guard for exactly this transition (`isBetweenInn`, shows `—` instead of a stale count) but that guard was never extended to the hero card or Due Up rail sitting right below it — so those two kept presenting a dead at-bat as if it were live, and (since team side was still derived from the stale `isTopInning`/matchup pairing) attributed it to the wrong team entirely once the half actually flipped.

**Fix:** added `_lgCurrentMatchup(feed)`, a shared helper sourcing the batter from `linescore.offense.batter` and the pitcher from `linescore.defense.pitcher`, with team side resolved by matching team id rather than `isTopInning`. Verified byte-for-byte against `currentPlay.matchup` during a genuine live at-bat (identical output) and confirmed correct during both transition states: Middle of the 2nd via a replayed real captured feed snapshot (`_lgCurrentMatchup` returned Michael Harris II / Anthony Molina, matching the live feed exactly), and End of the 3rd live in the browser minutes later (panel correctly showed San Francisco's Jung Hoo Lee due up with Osleivis Basabe/Andrew Knizner/Nate Furman next, not a leftover from Atlanta's inning). Also gated the hero card's "last pitch velocity" off `currentPlay.playEvents` during the between-innings window — that data belongs to the pitcher who just finished the previous half, not the one about to take the mound; it was being mislabeled as the incoming pitcher's, now suppressed instead. `sw.js` CACHE_NAME bumped v223 → v224.

**Not fixed in this pass, same root cause, lower exposure:** `_buildMatchupContent` (the opt-in "Matchup" tab's career-H2H + "This At-Bat" pitch log) reads `currentPlay.matchup` directly and would show the same stale pairing if opened during a Middle/End window. Left alone here since it requires an explicit tab click rather than being always-visible, and needs a bit more design thought (what "This At-Bat" should show when no pitch has actually been thrown to the new batter yet) rather than a drop-in swap to the new helper. Worth the same fix in a future pass.

**Escalation:** none — root-caused and live-verified same session.

---

## RETRACTED — "`.sl-rail` overlaps `.sl-primary` at mobile widths" was a false positive from a test-script bug, not a real site bug

**Contributor:** (session work, home redesign Phase 3) | **Date opened:** 2026-09-08 | **Date retracted:** 2026-09-08, same session

**What was originally reported here:** a claim that `.sl-primary` and `.sl-rail` render at the same `getBoundingClientRect().top` on any viewport ≤960px, making the sport-picker/Following/Recently-Viewed sidebar invisible on mobile, "confirmed on live production."

**Why that was wrong:** the CDP driver script used to check this (`cdp_drive.js`, a session scratchpad helper, not part of this repo) only applied its requested viewport width during a screenshot (`shot`) step — an `eval`-only check with no `shot` step ran against the script's hardcoded default viewport (1280×900) regardless of what width was intended. Every "mobile" measurement that produced the "identical `top`" result was actually taken at 1280px desktop width, where `.sl-layout`'s `grid-template-columns: 1fr 360px` is very much still active — `.sl-primary` and `.sl-rail` sharing the same `top` there is simply two side-by-side grid columns in the same row, which is correct, not a bug. The "production" confirmation was the identical mistake repeated against `sportstrata.cc`, not independent evidence.

**Actually verified, this time with the viewport genuinely set before evaluating:** at a true 390px width, `.sl-layout` correctly collapses to a single `358px` column, `.sl-primary` and `.sl-rail` land at different `top` values consistent with one stacking below the other (measured 286.75 and 3104.53, a ~24px gap apart from primary's own height — exactly `.sl-layout`'s `gap: 1.5rem` in its collapsed-column media query), and a full-page screenshot visually confirms the sport-picker cards render correctly stacked near the bottom of the mobile page. No overlap, no hidden content. The Following/Recently-Viewed sections were simply empty in that check (no follows/recents seeded in a fresh browser context) — expected behavior, not related to this.

**Lesson, not a site issue:** any future CDP-driven check must confirm the viewport was actually applied before an `eval` step it depends on, not just before a `shot` step — this exact script bug produced a fully-formed, internally-consistent-looking, "production-confirmed" false bug report. Verify tooling assumptions before trusting a measurement, especially one that contradicts a component that already screenshot-verified correctly (Phase 0/2/3 all did visually confirm mobile layouts earlier via `shot` steps, which were never affected by this bug — only bare `eval`-only checks were).

---

## MLB Pennant Races bar: a division leader with a dark primary color rendered as an invisible bar (White Sox and ~half the league) — found, root-caused, fixed 2026-09-08

**Contributor:** (owner report — "the white sox black percentage bar") | **Date:** 2026-09-08

**Reported:** the owner recalled a prior conversation about the Pennant Races division-odds bar (`_loadMLBLandingSignature`, `js/app.js`, the MLB sport-landing page's `#slSignature` module) rendering as a solid black/invisible bar for the White Sox. No record of the original report existed in DECISIONS.md, ISSUES.md, or memory — re-investigated from scratch against the current code rather than assumed.

**Root cause, confirmed both by computed contrast math and a live screenshot:** the bar-fill color (`.pr-bar-fill`, `background: var(--tc, var(--accent))`) uses `getMLBTeamColors(abbr).primary` directly with no contrast check. The White Sox's real brand primary is `#27251F` — a near-black charcoal — and the bar's track sits on the dark theme's actual composited card surface (`--bg-card`'s rgba layered over `--bg-base`, computed to ≈`rgb(19,23,28)`). WCAG contrast ratio between the two: **1.18** — visually indistinguishable, so a CWS-leading division showed what looked like an empty/broken bar.

**Turned out much bigger than the one reported case.** Computing the same ratio for every team's primary against the real composited card background found roughly **half the league** under a readable threshold: CWS/PIT (1.18), SD (1.19), NYY (1.18, despite already being flagged `darkSafe` for an unrelated *logo*-contrast reason — S5/D-047, a different problem this flag was never meant to cover), MIL (1.23), COL (1.18), TB (1.31), MIN (1.29), SEA/DET (1.30), HOU (1.33), TEX (1.48), NYM (1.39), CHC (1.58) — all effectively invisible bars, not just the one team that happened to get noticed and reported.

**Fix:** `js/config.js` gained real WCAG contrast primitives (`_parseCssColor`, `_compositeOverBg`, `_relLuminance`, `_contrastRatio` — ported from `tools/check-themes.cjs`'s own formula, ~15 lines, not worth pulling a Node tool into the browser for) and `_barSafeTeamColor(colors)`, which resolves `--bg-card` composited over `--bg-base` live via `getComputedStyle` and swaps to the team's `secondary` color when `primary`'s contrast ratio is under 1.6. Computed live per-theme rather than a hardcoded per-team list (unlike `darkSafe`) so it's correct across dark/light/nl-monarchs without hand-verifying each one, and it generalizes to any future team-color change without needing a new manual entry. Wired into `_loadMLBLandingSignature`'s one call site (`js/app.js`).

**Verified:** unit tests/manifest/theme-contrast all green; live screenshot of the real MLB landing page's Pennant Races module post-fix shows CWS (leading AL Central) with a clearly visible light-silver fill, HOU (AL West) with its bright secondary orange, TB (AL East) with light blue — all previously would have rendered as a near-black bar on a near-black track. `sw.js` CACHE_NAME bumped v255 → v256.
...
## NFL ticker falsely read "No NFL games today" on the actual 2026 season opener — found, fixed 2026-09-09

**Contributor:** (owner asked for a full NFL-workflow check ahead of kickoff) | **Date:** 2026-09-09, live-verified against the real Week 1 slate hours before SEA @ NE (9/9, 8:20 PM ET)

**Reported:** a full-workflow audit ahead of tonight's season opener. A live screenshot of the real production NFL Scores page showed the in-page ticker (`#scoreTicker`, `updateNFLTicker`) reading **"No NFL games today — check back soon"** directly above a Scores grid, with "Today" selected, showing tonight's SEA @ NE game as the very first card. The header ticker and sidebar (cross-sport home hero, `_sportPickerSnippet`) both correctly showed "NE 0 – SEA 0 · 8:20 PM ET · 1 today" at the same moment — only the NFL page's own in-page ticker was wrong.

**Root cause:** `fetchNFLScoreboard({})` — what the default "Today" tab actually calls (`js/nfl.js`, `loadNFLGames` with `_nflScoresFilter` null) — has no `dates`/`week` param, so ESPN's own `/scoreboard` endpoint returns its default: the **whole current week's** schedule, not literally today's slate (NFL has no real daily schedule the way MLB does; this is expected/correct ESPN behavior, not a bug). `updateNFLTicker(games)` then filtered that week-wide list down to `g.isFinal || g.isLive || g.homeTeam.score > 0 || g.awayTeam.score > 0` before deciding whether to show the idle message — a pregame game (0-0, not live, not final) never passed that filter, so on any day where today's game literally hadn't kicked off yet, the ticker saw zero qualifying games out of the full week's slate and fell through to the idle branch, even though a real game was hours away. MLB's equivalent ticker (`updateMLBTicker`, `js/mlb.js`) excludes pregame games too, but that's deliberately tolerable there (15 games/day, "so 0-0 SCH items don't flood the ticker," per its own comment) — for NFL, with often just 1-2 games on a given day, the same exclusion means the one game that matters gets dropped entirely rather than de-emphasized.

**Fix:** added `_nflIsGameToday(g)` (`js/nfl.js`), which compares a game's kickoff date against today's date using the same fixed ET-offset convention `_kickoffLabel` (`js/scorebug.js`) already uses for display, and added it as an additional pass condition in `updateNFLTicker`'s filter: `g.isFinal || g.isLive || g.homeTeam.score > 0 || g.awayTeam.score > 0 || _nflIsGameToday(g)`. A pregame game still shows once its kickoff is actually today (rendered via the existing `Scorebug.renderTickerItem`/`normalizeNFLGame`, which already had a "SCH + kickoff time" pill path that was simply never reached before); games later in the same week-wide fetch (Sunday's slate, seen on a Wednesday) correctly stay excluded, so the ticker doesn't flood with the whole week the moment any game exists. The idle "no games" message still fires correctly on a genuine gap day (e.g. a Tuesday of a bye week), since no game in that week's fetch would match today's date either.

**Verified:** local `wrangler pages dev` re-check post-fix showed a real ticker item (`data-game-id="401872656"`, ESPN's real event id for tonight's SEA @ NE) rendering with a "SCH" pill and kickoff time, replacing the idle message. Also checked (and ruled out as unrelated/pre-existing) a faint text-bleed-through artifact where the ticker's scrolling content passes behind the "SCORES" button while that button is the active nav tab (`.nav-tab.active`'s 10%-opacity background) — confirmed via DOM/computed-style inspection, not a screenshot guess, that this is sitewide (any sport, whenever its own ticker has scrolling content) and unrelated to this fix; NFL's ticker simply hadn't been observed in its scrolling state before now because this bug kept it permanently idle. Not fixed here — cosmetic, pre-existing, out of scope. `sw.js` CACHE_NAME bumped v259 → v260.
...
## SESSION HANDOFF — 2026-09-10 (clean shutdown, distribution-first pivot)

**Everything below is committed and pushed. Live-verified where noted. `sportstrata.cc/sw.js` confirmed at `sportstrata-v280` on production, matching local disk.** This session ran alongside unrelated concurrent work on `js/liveGame.js`/`css/liveGame.css` (win-probability graph, spray chart, leverage estimate — commits `3fccf62`, `0a62b36`) that this session did not touch and is not responsible for; mentioned here only so a future reader doesn't assume it's part of this handoff.

### How this session started, and why it matters for what's next
Owner opened by asking "what sport/feature next" and asked to be walked through a real brainstorm rather than handed a default answer. That surfaced two facts that changed the actual direction of the session: `GOALS.md`'s vision doc was stale (still described a two-sport MLB/NFL barbell months after six sports shipped — flagged, not yet re-ratified, still open), and the site's Cloudflare Web Analytics had been silently broken since its 2026-08-08 install — real traffic was running through an undocumented Cloudflare Pages dashboard auto-toggle instead (D-139, found and confirmed this session). Owner's own answers through the brainstorm converged on: the real goal is monetization, but sequenced behind distribution (traffic is the bottleneck, not surface area); content/social is explicitly not a channel the owner wants to invest in; soccer was named as an audience bet but conceded it doesn't actually solve the distribution problem. That sequencing — distribution before monetization — is the thread the rest of the session followed, and should keep being the default lens for prioritization until a future session/owner explicitly changes it.

### Shipped + live-verified this session (all decision-logged)
- **D-149 — Analytics instrumentation.** Removed the dead Web Analytics token from `index.html` (confirmed live: zero events ever, real collection was already running through the Pages dashboard auto-toggle, owner confirmed it's ON). Built a first-party, anonymous `page_views` D1 table (`migrations/0008_page_views.sql`) + `functions/api/track.js` + `_trackPageView()` in `js/navigation.js` (fires via `sendBeacon` on every `navigateTo()` call) — because Cloudflare Web Analytics has no custom dimensions and unconfirmed hash-routing SPA support, so it structurally cannot answer "which sport/page gets traffic" for this site. Migration applied to production `USER_DB` via the Cloudflare D1 dashboard console (wrangler CLI auth failed with a 403 from this environment — unresolved, see Owner to-dos below). Live-verified end to end: `/api/track` returns 204 on a valid write and 400 on an invalid payload, dead token confirmed absent from live `index.html`. Commit `c670dea`.
- **D-150 — Glossary gap.** Discovered `functions/glossary.js` (MLB stat glossary) was already built and in the sitemap (D-041 Phase 2) but had **zero internal links pointing at it anywhere in the live app** — indexed, but an orphan. Fixed with a contextual "What do these stats mean?" link on the MLB Leaderboards page. Built the NFL equivalent, `functions/nfl/glossary.js` → `/nfl/glossary` (ADP, VORP, PPR, Half-PPR, Standard, Superflex, SOS, Waiver Wire, Success Rate, Drive Efficiency, Next Gen Stats, Bye Week — each term checked against a real shipped feature before inclusion; deliberately excludes EPA/CPOE/target share/snap share since none of those are actually computed or shown anywhere on the site), linked from Draft HQ's "More Draft HQ tools" row. `tools/gen-sitemap.cjs` updated (not regenerated locally — needs outbound network the sandbox doesn't have; the existing daily GitHub Actions sitemap-refresh job will pick it up). Live-verified: both pages render correct titles/JSON-LD, both new links confirmed present in deployed `js/fantasy.js`/`js/mlb.js` (bypassing a stale Cloudflare edge-cache hit on the first check). Commit `8389fc0`.
- **D-151 — Monetization (D-069) revisited, deliberately not built.** Checked the real traffic number (1,790 pageviews/28 days) against Path A ad-network thresholds (Raptive 25k/month, Mediavine ~$5k+/year ad revenue) — more than an order of magnitude short. Recommended against sending any Path A/B outreach and against starting the Stripe subscription build this session; owner agreed in conversation. Full reasoning, and the specific number that would justify revisiting, in `DECISIONS.md` D-151.
- **NCAAB/WNBA team-detail templates — re-checked, confirmed still correctly blocked, not attempted.** Verified directly against `js/navigation.js`'s dispatch that no `ncaab-team`/`wnba-team` (singular, detail) view exists client-side, only the `-teams` grid views — same finding D-121 already made. Not rebuilt as a template against a view that doesn't exist.

### What the next session should do, in order
1. **Query the `page_views` table** (no dashboard built yet — direct query per D-149's own instructions in `CLAUDE.md`) once a few days/weeks of real traffic have accumulated. This is the actual input the "what sport/feature next" question needs — not available at the time this session ran.
2. **Revisit D-151's monetization hold** only once real monthly pageviews approach Raptive's 25k floor, or the owner makes a deliberate call to accept Path B's legal-review cost early regardless of scale, or supplies a pricing decision for the subscription tier. Don't re-do the research — `docs/sponsorship-outreach.md` and the ISSUES.md "AI League Insights — Paid Tier v1" three-gate spec are both still current and valid, just gated.
3. **`GOALS.md`'s vision section still needs re-ratification** — flagged this session (and originally flagged 2026-08-15 per the file's own note) as describing a two-sport strategy six sports have since outgrown. Not fixed this session; still open.

### Owner to-dos (nobody else can)
- **`wrangler` CLI is not usable from this sandbox for D1 operations** — `wrangler whoami` failed to resolve account IDs, and `wrangler d1 migrations apply --remote` returned a 403 (`code: 7403`, "account not valid or not authorized") even after that. Worked around this session by running the migration SQL directly in the Cloudflare D1 dashboard console instead. If CLI access is wanted for future migrations: check whether `$env:CLOUDFLARE_API_TOKEN` is set (would silently override `wrangler login`) and confirm the token has `D1:Edit` account-level permission, or confirm which Cloudflare account actually owns the `sportstrata-users` D1 database.

### Working conventions confirmed this session (unchanged, restated)
...
- **Sound design** — ✅ SHIPPED 2026-09-13 (D-159), synthesized tones (Web Audio API, not sourced files), off by default. The one piece of today's work that couldn't be verified against real data or heard in a real browser this session — flagged honestly in D-159, not asserted as equally solid as everything else.
- **Drive-history exploration** — ✅ SHIPPED 2026-09-13 (D-160), scoped from a full LIVE/DRIVE/GAME field-mode switcher down to a position bar added to the Play-by-Play tab's existing drive accordion, which was already doing the real exploration job.
- **Per-event-type push notifications, cross-sport engine abstraction** — all named, none started. Push notifications specifically needs backend Worker changes likely blocked on the same `wrangler` auth issue found this morning (D-149) — flagged before starting, not discovered mid-build.


---

## SESSION HANDOFF — 2026-09-13/14 (clean shutdown, full NFL Week 1 live-build session)

**Everything below is committed AND pushed AND live-verified. Last commit: `bf63400`. `sw.js` at `sportstrata-v291`, confirmed live via a direct cache-MISS check, not just a git push. Nothing pending.** This was an unusually long, single-arc session — full account below so a new session (or the owner) can pick up cleanly without re-reading the entire transcript.

### How this session moved, in order
Started as a broad "what's next" brainstorm (see the session's own opening exchange) that converged on distribution-first sequencing over adding a 7th sport. Shipped analytics instrumentation (D-149) and SEO glossary work (D-150) on that thread. Mid-session, the owner reported a real security incident (a browser-hijacking adware app, AdBlock360) — investigated, root-caused, and fully remediated (uninstalled, Chrome search engine reverted, full system scan clean) before returning to product work. NFL Week 1's first Sunday slate then became the focus: real live games running all afternoon/evening turned into an extended live-build-and-verify session on the NFL live game viewer specifically, producing the bulk of today's work (D-152 through D-160, eleven real features/fixes). Session closed with the owner directing a new focus — reviving NBA — recorded as D-161, with this handoff and a fresh-session kickoff prompt as the closing deliverables.

### Shipped + live-verified this session (all decision-logged)
- **D-149 — Analytics instrumentation.** Removed a dead Web Analytics token; built a first-party `page_views` D1 table + `/api/track` since Cloudflare Web Analytics can't give a per-sport breakdown for this hash-routed SPA.
- **D-150 — SEO glossary work.** New `/nfl/glossary` page; fixed the MLB glossary page being indexed but linked from nowhere in the app.
- **D-151 — Monetization (D-069) revisited, deliberately not built.** Real traffic numbers don't clear either sponsorship track's bar yet; recommended holding both, and the Stripe subscription build, until they do.
- **Security incident, same session, unlogged in DECISIONS.md (not a product decision) but real:** AdBlock360 (a browser-hijacking adware app installed 2026-08-24) found, root-caused, and fully removed — the app, its Windows service, scheduled tasks, and autostart entry all confirmed gone; Chrome's search engine reverted from a hijacked `read-news.net` provider back to Google and confirmed holding; full Windows Defender scan came back clean (zero detections). See the conversation transcript for the full investigation if ever needed — not duplicated into the decision log since it's a machine/security event, not a SportStrata product decision.
- **D-152 — Big-play Highlight Card auto-suggest**, shipped live during the actual Week 1 slate.
- **D-153 — Critical bug found in D-152 same day**, plus the leverage/tension ambient layer. The auto-suggest read `_nlg.situation` one level too shallow and had been silently non-functional in the real browser since it shipped hours earlier, despite passing every check run against it — found while building the tension layer, fixed same session.
- **D-154 — Sticky score header**, completing both fully-gated specs from the 2026-08-24 "immersion brainstorm."
- **D-155 — Key Plays** (win-probability-swing ranking) and **D-156 — Catch Me Up** (a since-you-left recap), both NFL-only, zero new data sources, both live-verified against real Week 1 plays.
- **D-157 — Full-page "critical moment" overlay** — a genuine, owner-approved departure from this file's usual restraint, scoped narrowly (a real, rare >=15-point win-probability swing) and calibrated against all 10 real games played that Sunday (8 total qualifying plays site-wide) before shipping.
- **D-158 — "Why It Matters"** deterministic (non-AI) game-state explanation, added to the critical-moment overlay.
- **D-159 — Sound design**, synthesized Web Audio tones (not sourced files), off by default. Flagged as the one piece of this session's work that couldn't be verified against real data or heard in an actual browser — a genuine, disclosed limit, not asserted as equally solid as everything else.
- **D-160 — Drive-history position bars**, added to the existing Play-by-Play accordion rather than building a duplicate drive list.
- **A real Cloudflare platform-side deploy failure**, diagnosed from the build log (not guessed): `92c781b`'s build cloned/compiled/uploaded assets successfully and failed only on the final Functions-publish step with a generic Cloudflare-side "Unknown internal error occurred" — confirmed unrelated to that commit's actual file changes. Resolved with a fresh triggered build (`bf63400`), confirmed live afterward via a direct cache-MISS check against the origin, not just trusting the push succeeded.
- **D-161 — Owner directs NBA revival as a live 7th sport.** Checked against D-052's prior (twice-ratified) analysis before writing anything new rather than treating it as a fresh question — see D-161 for the full record.
- **G6 (GOALS.md) re-ratified**, closing a real, previously-flagged-but-unfixed doc-drift (the "two-sport barbell" framing hadn't reflected reality since NCAAF shipped in July) in the same pass as recording the NBA decision.

### What the next session should do, in order
1. **Read the fresh-session kickoff prompt** (delivered directly to the owner at the end of this session, not duplicated verbatim here) before starting any NBA work — it already encodes D-052's engineering findings and the specific open nav/home-capacity question the owner wants investigated first.
2. **Do NOT start NBA implementation before measuring real nav capacity**, per D-161's own explicit deferral — this project has hit real, live nav-overflow bugs at every sport-count increase so far (D-111's mobile sport-switcher rework at 5 entries is the most direct precedent). Measure before building, matching D-070's own established audit method.
3. **Decide (with the owner, not unilaterally) whether NBA reuses/patches its old pre-DESIGN.md component code or is rebuilt fresh** matching the NCAAB clone-and-adapt pattern — D-161 deliberately left this open rather than presuming an answer.
4. Everything else from tonight's "elevate the experience" arc that's still open (per-event-type push notifications, the cross-sport tension-layer abstraction) remains exactly where it was left — not blocking, not forgotten, just not picked up this session.

### Owner to-dos (nobody else can)
- None blocking. The `wrangler` CLI auth issue found this morning (broken API token, D-149) is still unresolved but has a working fallback (the Cloudflare dashboard) for anything that needs it — not urgent, just still true.
- If today's Cloudflare platform-side deploy failure (`bf63400`'s trigger) recurs, the same fix applies: check the Deployments tab's build log for the actual failure point before assuming it's a code problem — today's specific failure was confirmed to be Cloudflare-side, not caused by anything shipped.

### Working conventions reinforced this session, worth restating
- **A live event is a genuinely scarce resource for live-feature work** — several of tonight's builds (the big-play auto-suggest, the critical-moment threshold calibration) could only be meaningfully verified because real NFL games were actually in progress. Worth deliberately sequencing similar work around live windows again, not just building blind and hoping.
- **Hand-tracing rules against raw API data is necessary but not sufficient** — D-153's critical bug (a wrong variable path) passed every rule-level verification done against real data, because the verification never exercised the actual JS variable path connecting that data to the running code. Test the wiring, not just the logic, especially through a wrapped/nested return shape.
- **Check what already exists before building a parallel version of it** — happened twice today: the "evergreen glossary" idea (already shipped, would have been duplicated) and the drive-history field mode (the Play-by-Play tab was already doing the real job; the actual gap was narrower than the original ask implied).
