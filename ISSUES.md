...
**Still open before production:** confirm `nodejs_compat` is set in the Cloudflare Pages dashboard itself (Production + Preview — the local `wrangler.toml` flag only covers `wrangler pages dev`, not the dashboard-built live site); task #130 follow-star wiring; a full `/security-review` before this goes anywhere near production traffic.
