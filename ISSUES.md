...
## YouTube Channel Analytics Dashboard — owner-only gated route — Three Gates

**Contributors:** Vera (behavioral), Kael (visual), Axiom (feasibility) | **Date:** 2026-08-09

**Context (owner):** "let's brainstorm how we can connect the SportStrata YouTube, maybe we could create a simple analytics dashboard that just deploys on the dev side." Investigated before speccing: this repo's `bot/content_engine.py` generates NFL/NCAAF Shorts packages and the separate `sportstrata-video` repo runs a full long-form episode pipeline ("Draft Instincts," D-054, already past M1) — both feed the channel, neither reads anything back. Owner chose a gated route on sportstrata.cc over a local-only script or a separate internal deployment (full reasoning in DECISIONS.md D-082).

**Job to be done (Vera):** not generic channel vanity metrics — "did the content this operation produces actually work." The owner needs, per video: performance (views, retention, subscriber effect, traffic source) and which pipeline/content type/sport it belongs to, so future `content_engine.py` and Draft Instincts output can be prioritized toward what's proven to work rather than produced blind.

**Behavioral spec (Vera):** New gated view, reachable and rendered only for the owner's own signed-in account — a single hardcoded email check, not a new role/permission system (this is a channel of one, building generic RBAC here would be solving a problem that doesn't exist). States: **signed-out/non-owner** (view doesn't render at all, no hint via error copy that a hidden admin surface exists — same discretion any credential-gated page should have), **no-sync-yet** (before the first cron run completes — an honest "no data synced yet" empty state, never a fabricated zero), **populated** (channel headline numbers + a per-video table), **sync-stale** (a small "as of {time} ago" freshness badge if `synced_at` is older than one sync cycle — the receipts pattern this product already uses everywhere, not new here), **per-video detail** (expandable row for traffic-source breakdown, reusing the `<details>`/`.nlg-card` collapsible pattern already proven in the NFL Live Game Viewer this same day). Per-video table is sortable/filterable by views, average view percentage, subscribers gained, and content-source tag. **Content-source tagging is shown as inferred, not authoritative** — Shorts vs. long-form is a reliable YouTube-native signal (duration under 60s), but the sport tag is a title-keyword heuristic and must say so visibly, same "label it honestly" discipline the injuries/news surfacing work just established for the NFL news field.

**Visual spec (Kael):** No new component family — reuses the `.nlg-card`/`.nlg-side-card` collapsible and stat-row shapes already proven in the NFL Live Game Viewer, `--font-display`/tabular-nums for stat values per the standing numeric-voice rule. Deliberately **less** polish investment than any public-facing feature — this is a one-person internal tool, not a product surface, and shouldn't receive the same skeleton-loader/empty-state-copywriting bar the public site's rules demand elsewhere. No brand-orange decoration beyond the existing "brand only" invariant; if anything this page should read quieter than the public site, not louder — it's a utility, not a showcase.

**Feasibility (Axiom):** New D1 table `youtube_video_stats` on the existing `USER_DB` binding (no new database provisioned): `video_id` (PK), `title`, `published_at`, `duration_seconds`, `content_source` ('shorts'|'longform'), `sport_guess`, `views`, `avg_view_duration_seconds`, `avg_view_percentage`, `subscribers_gained`, `subscribers_lost`, `likes`, `comments`, `traffic_source_json`, `synced_at`. New cron Worker `worker/youtube-analytics-sync.js` + `wrangler-youtube.toml`, same shape as `worker/push-game-alerts.js` (D-079) — daily schedule, refreshes an access token from a stored OAuth refresh token, calls YouTube Data API v3 (`videos.list`) and YouTube Analytics API (`reports.query`), upserts into D1. New Pages Function (e.g. `functions/api/admin/youtube.js`) reads D1 and checks the session server-side against the owner's account specifically before returning anything — mirrors the "server is authoritative" pattern already enforced on every D-031/D-064/D-065/D-069 endpoint, narrowed from "any signed-in user" to one specific account, which no existing gated view in this codebase currently does. Client view wires into `navigation.js`'s existing gated-view pattern with that same narrower check. **No CSP change needed** — the client never calls Google directly; only the Worker does, server-side, so `index.html`'s CSP meta tag and `_headers` are untouched (checked explicitly since other features have hit this exact requirement). Requires three new secrets — `YOUTUBE_CLIENT_ID`, `YOUTUBE_CLIENT_SECRET`, `YOUTUBE_REFRESH_TOKEN` — via `wrangler secret put`, never in source. Quota checked live 2026-08-09: Data API v3's free tier is 10,000 units/day, Analytics API has its own separate pool; a once-daily single-channel pull is nowhere near either limit.

**Gate status: all three gates present.** Blocked on one owner action before real-data verification is possible: the Client ID is already created; the Client Secret and a refresh token (minted via a one-time Google OAuth Playground authorization) still need to be generated and stored via `wrangler secret put`. That step is orthogonal to writing the code, not sequentially blocking it — the D1 schema, Worker, and gated Function/view can all be built now and are ready for a build pass.
