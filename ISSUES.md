...
**Not yet actioned (owner input needed on priority):** regenerating the sitemap (owner-run), the rolling game-page discovery window, dynamic per-page `og:image`, and NFL/NCAAF leaders+game path URLs are all real, scoped findings — none implemented this pass pending a priority call.
