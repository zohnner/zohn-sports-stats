...
### N-17 — NFL injury report — **[Vera]** · priority 3 · QUEUED, not started
**Scoping decided 2026-08-02** (2nd of 3 owner-directed "NFL expansion" initiatives, sequenced one at a time after N-16): a dedicated injury-report surface, separate from the waiver wire below (owner's explicit call — not a combined "roster moves" hub). Not starting cold: `p.injury_status` from the Sleeper pool already flows through `_nflPoolMap` and already renders inline as a badge on roster rows (`.roster-il-badge`, `_renderNFLTeamDetail`) and on Top Fantasy Assets cards (`.team-asset__inj`) — so the data source and a working badge pattern both already exist. What doesn't exist yet: a dedicated view that surfaces injury status *across* players/teams (e.g. "who's questionable this week league-wide") rather than requiring a fan to already be on a specific team or player page to see it. Needs a Vera pass next — job-to-be-done, whether this is a standalone nav destination or a filter/tab on an existing view (Players? Teams?), and what states beyond "has injury_status" vs "doesn't" are worth showing (Sleeper's field is a raw string like "Questionable"/"Out"/"IR" — worth checking the full value set before designing states around it).
