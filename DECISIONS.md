...
**Status:** Phase 1 (buildable-now slice) shipped, committed locally, not yet pushed/deployed. Win-expectancy table scoped as a follow-up, not built. Second data source (deeper Statcast/Savant) explicitly not started.
...
**Not yet live-verified after deploy.** This session cannot push (no GitHub auth) or run a local dev server the desktop bridge's Chrome can reach — the live-Chrome verification in this entry is all *pre-change* (auditing the deployed page as it existed before these edits), not a check of the new layout/pregame/sidebar code in a real browser. Per this project's own established discipline for changes that can't be screenshot-verified from a session (D-005, N-5's "implementation deferred to a verification-capable session"), this pass is being logged and committed rather than silently treated as done: **owner needs to `git push`, then this needs a live Chrome pass against real production** — a live game for the width/sidebar/base-state/spin-tooltip changes, and ideally a genuinely pregame matchup (or a forced-preview console check against a real `feed/live` payload if none is available live) for the pregame state, before this entry's status moves past "shipped, not yet deployed-verified."
