...
## D-031 — Accounts, retention & monetization foundation — Phase 1 LIVE-VERIFIED LOCALLY 2026-08-04 (remote migration + security-review still open)
...
**Not done yet:** (a) the remote D1 migration (0003, the rate-limit table) hasn't applied — `wrangler d1 migrations apply --remote` is currently failing on a Cloudflare API authorization error (`code: 7403`) unrelated to the migration itself; production will 500 on every auth request exactly like local did until this is resolved and confirmed, (b) confirm `nodejs_compat` is set in the Cloudflare Pages dashboard itself (Production + Preview — the local `wrangler.toml` flag doesn't cover the dashboard-built live site), (c) wire follow stars onto the existing card templates (deferred, ISSUES.md task), (d) a full `/security-review` gates this before it goes anywhere near production traffic.
