...
**Gate reopened 2026-08-20 (D-116) — the missing fields are no longer missing.** A live `feed/live` payload fetched during a real in-progress game (NYY@BAL, gamePk 824801) confirmed `pitchData.coordinates.pfxX`/`pfxZ` and a full `pitchData.breaks` object (`breakAngle`, `breakVertical`, `breakVerticalInduced`, `breakHorizontal`, `spinRate`, `spinDirection`) populated on real pitches, plus `pitchData.zone` and `plateTime`. This entry's own gating condition — "until the owner supplies a real `feed/live` sample so the fields can be verified" — is satisfied. Spin rate is now surfaced in the pitch tooltip (D-116, shipped) as a first, small use of this data; the animation itself is not built and needs its own Kael/Vera pass before Finn starts (see D-116).
...
## D-114 — Home page redesign: external-LLM roadmap triage; zones/marquee/Starters-grid/stat-bars shipped, two items declined, two deferred
**Status:** partially shipped (build complete, committed locally, not yet pushed); two items declined on standing-constraint/product-scope grounds, two deferred pending data/owner input
**Contributors:** Finn (fact-check), Vera (JTBD/states), Kael (visual), Axiom (build + live verification)
**Date opened:** 2026-08-19 | **Date resolved:** 2026-08-19 (build); open on the declined/deferred items

**Trigger:** owner pasted a 5-phase home-page redesign roadmap written by another LLM and asked the team to route it through the normal process rather than build it directly — the same situation as the original ChatGPT-brief episode (see ISSUES.md, "Home — Remaining ChatGPT-brief ideas, scoped not built," and D-091). After the triage, owner said to continue with the team's scope and implement whatever they deemed worthy.

**Finn's fact-check, before any persona weighed in:** Phase 1's "move the tweet/social embed out of the hero" doesn't apply to this codebase — there is no social/tweet embed anywhere in the hero or the page body (confirmed directly against `index.html`; the only Twitter-related markup is unrelated `twitter:card` link-preview meta tags in `<head>`). Dropped, not built — same category of stale-premise error the original ChatGPT brief had (D-090).

**Triage outcome (full per-item table given to the owner in chat before this entry):**
- **Declined — conflicts with a standing constraint, not overridden by "implement anything worthy":** Phase 1's ticker condensing ("single Scoreboard row, live-games-only"). `ux.md`'s "Do not redesign" list names the score ticker explicitly (live scores are primary chrome, not secondary; the ticker's SCORES button is the mobile nav entry point for `mlb-games`), and this would also unwind D-087's merged cross-sport ticker (followed-team pinning, 30s poll loop) and remove the only Scores-nav path NCAAF/NCAAB currently have on mobile. A general "build what's worthy" mandate doesn't clear a named standing constraint on its own — that needs its own dedicated decision if it's ever revisited, not a line item inside a broader redesign pass.
- **Declined — real product/compliance question, routed to the owner separately, not built:** Phase 5's "SportsStrata Picks" and "Lock" high-confidence-play badges. GOALS.md's Non-Goals state "Not a pure betting site — odds as contextual reference only," and any sportsbook-adjacent work already requires real legal/licensing review per D-069. An in-house picks/predictions engine, presented as a "high-confidence play," is a materially different product category than referencing external odds — a design-team consensus isn't the right body to clear that, regardless of how buildable it is.
- **Deferred — no data to build on yet:** Phase 1's hero win-probability insight and Phase 4's VS win-probability bar. Checked the codebase directly rather than assuming: the only win-probability implementation anywhere in this repo is NFL's (`js/nflLiveGame.js`, D-106, via ESPN `/summary`'s `winprobability[]`) — nothing equivalent exists for MLB. This needs a Relay data-availability pass, not a UI decision, before it's buildable.
- **Not attempted this pass:** Phase 2's literal DOM reordering (moving Pennant Races/Starters/Insights into the roadmap's proposed grouped order). Real risk for a same-session change; the actual documented complaint behind Phase 2 (no visual separation between flat modules) is addressed below without touching order.
- **Built this pass:** Phase 2's visual-separation fix (light version), Phase 3's marquee treatment + card polish, Phase 4's Starters grid, and the non-betting half of Phase 5 (data-driven ERA/K-9 bars). Detail below.

**What shipped (`js/app.js`, `css/main.css`):**
- `.home-zone` (new): shared container (`--bg-surface` background, `--border-default` border, `--radius-lg` radius — no new tokens invented) applied to Today's Games, The Latest (headlines/insights rail), Tonight's Starters, and Hot Right Now in `loadHome()`'s template. Gives the page distinct thematic blocks instead of one flat scroll — the actual, valid complaint behind Phase 2 — without moving any section's order or internal DOM, so nothing downstream (ids, `gridEl.parentNode` lookups in `_loadHomeTodayGames`, etc.) needed to change.
- Today's Games "marquee" treatment (`.home-game-card--marquee`, wired in `_loadHomeTodayGames()`): the first 3 rendered game cards — already rank-sorted (favorite+live/today first, then live, then the rest) — get a 2-column grid-span with bigger logo/score type. Deliberately no accordion and nothing hidden: this resolves the tension Vera flagged between Phase 3's "hide most games behind Show All" idea and D-091's rationale for keeping D-088's single dominant hero ("Today's Games is already the comprehensive, everything-visible view") — that rationale only holds if Today's Games stays fully visible, so it stays fully visible.
- Tonight's Starters grid (`.sp-grid`): was `flex-direction: column` — one full-width matchup per row, up to 8 stacked, the real "vertical space killer" the roadmap named. Now a responsive grid, `grid-template-columns: repeat(auto-fill, minmax(min(420px, 100%), 1fr))`. Two real bugs caught live before shipping, both fixed same pass — see verification note below.
- ERA/K-9 mini-bars (`.sp-stat-bar`, computed in `_spCard()`): small team-colored horizontal bar backing the existing ERA and K/9 numbers. Fixed reference scale (ERA 1.00→6.00 inverted since lower is better, K/9 0→14 direct), clamped against outliers. Decoration only, not a new claim — the printed number stays the source of truth (DESIGN.md's receipts pattern) — and inherits the pre-existing `@media (max-width:480px) { .sp-statline { display:none } }` rule untouched, so it's automatically absent on the smallest screens same as the rest of the stat line.

**Verified live before shipping (Chrome against real production, same methodology this project already uses for every home-page change — not a local mock):** injected the patched CSS/JS behavior directly into `sportstrata.cc`. Screenshot-confirmed the marquee treatment (3 visibly larger, correctly-ordered cards, no layout break) and the zone containers (a real but deliberately subtle border/background separation — a loud box would itself be a DESIGN.md minimal-chrome violation, so subtle is correct, not a rendering shortfall).

**Two real bugs found and fixed during this same live-verification pass, before anything was written back to the repo:**
1. First attempt used `minmax(280px, 1fr)` for the Starters grid. Live screenshot showed real names truncating ("Wilkinson" → "Wilkin...", "Liberatore" → "Liberato...") and the opponent-history rows (BAA/K/BB, already part of the existing card, not added by this pass) crowding badly at that width. Widened to 420px, re-screenshotted clean.
2. A bare `minmax(420px, 1fr)` floor would force a >420px single column — real horizontal overflow — on any viewport narrower than that. Chrome's window-resize tool didn't affect this session's fixed-resolution display, so this was proven with a synthetic 375px-wide container injected into the live page instead: the bare-420px version reported a 375px container computing a 420px column (confirmed overflow); switching to `minmax(min(420px, 100%), 1fr)` reran the same synthetic test and correctly collapsed to a single 375px column with no overflow.

**Verified locally:** `node --check js/app.js` clean, 0 NUL bytes on both changed files, `tools/check-manifest.cjs` clean (0 failures/0 warnings), `tools/check-themes.cjs` clean (2 pre-existing unrelated warnings only — light/nl-monarchs `--accent`-on-`--accent-subtle` contrast, unchanged by this pass), full 48-test unit suite (`stats`, `vbd`, `query`, `odds`, `entitlement`, `scorebug`) passing.

**Not yet live-verified after deploy** — committed locally via the mount-safe git-plumbing workaround; no push access from this session, owner needs to `git push`. Next check: confirm zones/marquee/Starters-grid/mini-bars render correctly on production, and spot-check the Starters grid on a real narrow device (not just the synthetic-container test above, which proves the CSS math but isn't a substitute for a real phone).

**Open, for the owner or a future session:**
1. Ticker condensing — declined by the team on standing-constraint grounds; only reconsider via its own dedicated decision.
2. "SportsStrata Picks"/Lock badges — needs an explicit owner call on product scope and compliance, kept separate from this visual work on purpose.
3. Hero/Starters win-probability — needs a Relay data-availability pass (no MLB win-probability source currently exists anywhere in the codebase) before it's buildable.
4. Phase 2's actual DOM reordering — not attempted; today's fix addresses the same underlying complaint (no visual zones) at lower risk. Revisit only with its own Vera/Kael pass if the lighter fix turns out not to be enough.

**Escalation:** none — triage, build, and live pre-push verification all completed same session, same bar as every other home-page change this project has shipped. Push + live post-deploy verification remain.

**Live post-deploy verification found two real bugs, both fixed same day (2026-08-19), same session as the owner's push:**

1. **Avatar photo/initials stacking bug (pre-existing, not introduced by this pass, but only noticed once the Starters grid drew attention to the section).** Real production screenshots from the owner showed pitcher headshots with their 2-letter fallback initials ("JS," "SL," etc.) visibly overlaid on top of the loaded photo, on every card. Root cause, confirmed via `getComputedStyle`: `.sp-avatar img` is `position:absolute` with `z-index:auto`, while `.sp-avatar-initials` is `position:relative` with an explicit `z-index:0` — at the same effective stacking level, the later DOM sibling (the initials span) paints on top regardless of whether the image loaded successfully. Confirmed live (`img.complete === true`, `naturalWidth: 213` — a real loaded photo, not a broken one) that this reproduces even when the photo is fine. Fix: `z-index: 1` on `.sp-avatar img`, so a loaded image always paints above the fallback; the fallback still shows correctly whenever `data-hide-on-error` removes a failed image, since nothing then covers the initials.
2. **ERA/K-9 mini-bar contrast bug + general Starters-card legibility.** The bars used the pitcher's own team color (`colors.primary`) as fill — reads fine for a bright team, nearly invisible for a dark one (confirmed live: the Athletics' `#003831` green essentially disappeared against the dark card background). Fixed by switching the fill to a 3-tier quality-threshold color (`_statQualityColor()` in `js/app.js`: elite `--color-win` green, poor `--color-loss` red, average `--text-muted` neutral) instead of team color — this both fixes the contrast bug and adds real signal, consistent with DESIGN.md's existing rule that thresholded values may borrow win/loss color (the same rule D-096's red-zone coloring used). Also bumped the bar itself (22px→26px, 5px→6px), `.sp-statline`'s font-size (0.66rem→0.72rem), and `.sp-info`'s line gap (0.1rem→0.22rem) for general legibility at the Starters grid's card width — the owner's direct feedback was that the stats read as cramped/hard to read, and the tight 0.1rem line gap plus small type were real contributors independent of the two bugs above.
...
## D-116 — MLB Live Game Viewer overhaul: page-mode width/sidebar rebuild + pregame state shipped; win-expectancy spec'd separately, trajectory-animation gate reopened, second data source held off

**Status:** Phase 1 shipped, pushed, and partially live-verified on production (pregame state confirmed correct 2026-08-20). One live-caught follow-on bug (tab-strip `[hidden]` cascade) fixed and committed locally, not yet pushed. Live-state behavior (base persistence, spin tooltip, sidebar mid-game updates) still unverified — no MLB game has been live during any check so far. Win-expectancy table scoped as a follow-up, not built. Second data source (deeper Statcast/Savant) explicitly not started.
**Contributors:** Relay (data audit), Vera (job-to-be-done + states), Kael (visual direction), Axiom (feasibility + implementation), owner (scope decisions)
**Date opened:** 2026-08-20 | **Date resolved:** 2026-08-20 (Phase 1)

**Trigger:** Owner (Cowork): overhaul the MLB live game viewer to be genuinely competitive with ESPN, MLB.com Gameday, and theScore — not an incremental tweak. Two hard rules given: verify every claim live against a real in-progress MLB game via Chrome (no mocked/stale data), and audit the current data source before designing any UI rather than assuming MLB Stats API is sufficient or that a second source is needed.

**Precedent checked first, not assumed:** D-009 (Live Game Expanded View) is closed, not open — the "inline accordion vs. full page" deviation flag resolved 2026-06-09 with the full-page `showMLBLiveGame` accepted as the intended direction. Nothing in this pass reopens that; the full-page entry point stays. D-080 (NFL Live Game Viewer rebuild) turned out to be the inverse of what the brief assumed: NFL's tabbed + section-diff-render architecture was *ported from* MLB's `js/liveGame.js` (D-009's own Phase 1 pattern), not the other way around — MLB already had diff-based polling (9s linescore poll, `feed/live` fetched only on state change) and tab-panel switching before NFL did. So this was not an architecture rebuild; it was a scope-and-density problem plus two real bugs.

**Live verification, all against a real in-progress game (NYY@BAL, MLB gamePk 824801, ESPN eventId 401816589; checked live from the bottom of the 6th through the top of the 7th, 2026-08-19):**

**Relay — data audit.** The only source in use is MLB Stats API direct-`fetch()` through the Cloudflare proxy (`v1/game/{pk}/linescore` diff-polled every 9s, `v1.1/game/{pk}/feed/live` fetched only on state change, `people/{id}/stats?stats=vsPlayer` for the Matchup tab). Confirmed live: idle polling produced exactly one linescore request in 10 seconds with no play — the diff-based design works as documented. Fetched the real live `feed/live` payload directly and confirmed present, populated, and completely unused by the pre-existing UI: `gameData.weather` (condition/temp/wind), `gameData.venue.name`, `gameData.probablePitchers`, `gameData.gameInfo.firstPitch`, `gameData.officialScorer`, `gameData.moundVisits` (used/remaining per team), `gameData.review` (challenge/replay used-remaining per team), `liveData.boxscore.officials` (umpire crew by position). One field checked and found **not** to be what it looked like from the key name alone: `liveData.leaders` is not a game-leaders module (unlike what a first pass assumed) — it is `hitDistance`/`hitSpeed`/`pitchSpeed` tracking, and was empty (`{}`) on the game checked. Excluded from the sidebar for that reason rather than built against an unverified/likely-empty shape. **Reverses part of D-009's 2026-06-12 amendment:** that amendment gated pitch-trajectory animation because `pfxX`/`pfxZ`/`breaks.*` were "not confirmed" in the live payload. A real pitch from tonight's game has `pitchData.coordinates.pfxX`/`pfxZ` and a full `pitchData.breaks` object (`breakAngle`, `breakVertical`, `breakVerticalInduced`, `breakHorizontal`, `spinRate`, `spinDirection`), plus `pitchData.zone` (standard 1-14 Statcast grid) and `plateTime` — every field the amendment named as missing is present. Confirmed via full-payload string search: **no win-probability field exists anywhere** in MLB Stats API's feed — consistent with D-009's original finding. ESPN's Gamecast win-probability chart is their own model, not something MLB Stats API exposes.

**Vera — job-to-be-done + states.** Live job: "I have this game on in another window and want to glance at SportStrata to see what just happened and what's about to." The pre-existing page served this narrowly (play log updated correctly, pitching-change note appeared, scores didn't lag) but not the sustained "second-screen production truck" job the brief asked for, because there was nowhere to look besides the log. Pregame state did not exist at all — `probablePitchers` was fetched and discarded. Base state fully disappeared between at-bats (confirmed live: vanished the moment a fresh batter stepped in with zero pitches thrown) rather than showing empty bases — should never fully hide once real play context exists. No live games / postponed states were not live-observable this pass (nothing postponed or off-day tonight to check against) and are not addressed here — need their own spec pass against a real payload before Finn builds them.

**Kael — design direction.** Root cause of the page reading as thin, found in code, not guessed: `css/liveGame.css`'s `.lg-live-page { max-width: 680px; margin: 0 auto; }` is the inline accordion's own width, carried unmodified into the full-page route. Opened live on a 2504px viewport and got a ~400px card floating in mostly empty black — not a perception issue, the file. Violates DESIGN.md's own posture ("Baseball Savant crossed with a broadcast lower-third system... dense tables are the product") on the one page that should be its hardest test, same framing D-080 used for NFL's rebuild. Also caught live, not cosmetic: the page rendered the literal text `mlb-live-824801` above the header on every load. Root cause read in `js/navigation.js`: `setBreadcrumb(view, null)` looks up `_NAV_META[view]`; dynamic routes like `mlb-live-{gamePk}` were never registered there, so it fell back to `{ label: root }` and printed the raw hash. `js/nflLiveGame.js` and `js/scorecard.js` both self-call `setBreadcrumb(...)` to override this exact fallback; `js/liveGame.js`'s `showMLBLiveGame()` never got that call added — a real bug, live since D-009 shipped, not merely a stylistic gap.

**Axiom — feasibility + triage.** Split into: buildable now with no new data source (shipped this pass — see below); needs a short data-design pass, not blocked on external data (win-expectancy table — Relay builds/sources a static empirical model from inning/score-differential/outs/base-state, a small well-documented state space, not the multi-week nflverse-ingestion problem NFL's EPA work is); needs an owner decision (whether to lean harder on a second data source — held off, see Decision below).

**Decision (owner, via structured Q&A after the audit):**
1. **Win expectancy:** spec it now, build separately — Relay gets a follow-up spec to source/validate the empirical win-expectancy table; does not ride along with or block this pass's shipped slice.
2. **Pitch trajectory animation:** reopen the D-009 gate now (done, in D-009's amendment section above), defer the actual build — Kael/Vera give it its own pass before Finn builds it.
3. **Second data source (deeper Statcast/Savant):** hold off — ship the free wins from the unused-field audit first, then re-compare against ESPN/Gameday before deciding whether a new source is actually needed.
4. **Process:** log this entry now and hand the buildable-now slice to Finn — done in the same pass (Axiom implemented directly rather than a separate handoff, given the scope was already fully specified by the three gates above).

**What shipped (`js/liveGame.js`, `css/liveGame.css`):**
- **Breadcrumb fix.** `showMLBLiveGame` now self-calls `setBreadcrumb('mlb-games', '{away} @ {home}')` (best-effort from the game stub immediately, refined from the feed once it loads), matching the pattern already proven in `nflLiveGame.js`/`scorecard.js`. New module flag `_lgIsPageMode` distinguishes the full-page route from the (unwired) inline accordion so this only fires in page mode.
- **Full-page width/sidebar rebuild.** `.lg-live-page` widened from a 680px single column to a 1180px two-column grid (main panel + 300px sidebar), collapsing to one column under 900px (matching the responsive posture D-080 named for NFL's own sidebar — "no sidebar real estate under ~900px"). New `_buildSidebar(feed)` renders three receipts-style cards from data already fetched by every poll — Game Info (venue, weather condition/temp/wind), Game Notes (mound visits + challenges remaining, per team), Umpires (full crew by position) — zero new requests. This is real broadcast detail neither ESPN's Gamecast nor MLB Gameday's live page shows today; not a copy of either competitor, an area where the unused MLB Stats API data already outdoes both.
- **Pregame state.** New `isPreview`/`isLive` branches in `_renderPanel`: badge shows first-pitch time (`SCHEDULED` fallback) instead of `LIVE`/`DELAYED`, the inning/count pill is suppressed until the game is actually live, the Highlight Card link is hidden (nothing to highlight yet), and the tab strip is replaced by a new `_buildProbablePitchers()` block reading `gameData.probablePitchers` — names only in v1, no season stat line (would need a new per-pitcher fetch not scoped this pass; shipped honest and minimal rather than guessing at unverified data). `_lgNextInterval(feed)` (new helper, replaces inline re-arm logic in `_doPoll` and the hardcoded `LG_POLL_MS` first-arm in `showMLBLiveGame`) polls a pregame page at `LG_PREGAME_MS` (60s) instead of the live 9s cadence — a real inefficiency found while adding pregame support, not present in the original bug list, fixed in the same pass.
- **Base state never fully hides.** `_renderZone` previously hid the entire pitch-zone column (bases included) whenever the current at-bat had zero pitches yet. Now only hides fully pregame or when there's no play context at all (`!currentPlay`); a fresh at-bat with no pitches shows the base diagram with a "Next pitch coming up" placeholder in place of the pitch plot, rather than nothing.
- **Spin rate + zone number surfaced.** `_buildPitchZone`'s pitch dots now carry `breaks.spinRate` in their aria-label and tooltip (`data-spin`), joined with velocity in `_lgShowTooltip`. First, small use of the D-009-amendment-reversing field confirmation above — not the trajectory animation itself, which stays a separate future pass per Decision #2.

**What did not ship, on purpose:** win-expectancy/win-probability (Decision #1 — separate Relay spec first); pitch-trajectory animation (Decision #2 — gate reopened, build deferred); any second data source integration (Decision #3 — held off); no-live-games-today/postponed states (Vera — not live-observable this pass, needs its own spec against a real payload); probable-pitcher season stats in the pregame block (Axiom — would need a new fetch, out of this pass's scope).

**Verified locally:** `node --check` clean on `js/liveGame.js` and every other `.js` file in the repo (full sweep, not just the changed file); 0 NUL bytes on both changed files; full existing unit suite (`tests/stats.test.js tests/vbd.test.js tests/query.test.js tests/odds.test.js`, 33/33) passing — none of it exercises `liveGame.js` directly, so this confirms no collateral breakage elsewhere, not new coverage of this change. `tools/check-manifest.cjs`/`tools/check-themes.cjs` not run this pass (no script-chain or token changes) — should run before push per the standing `/deploy-check` discipline. Cascade-safety check done: every new CSS selector (`.lg-side-card`, `.lg-side-row`, `.lg-side-val`, `.lg-side-line`, `.lg-side-line--muted`, `.lg-pregame-wrap`, `.lg-zone-empty`, `.lg-sidebar`) grepped against the full `css/` and `js/` trees before adding — no collisions found; `.arcade-back-row`'s existing flex styling confirmed compatible with the new grid-column placement.

**Live-verified after deploy (2026-08-20, real production, STL@CIN, gamePk 824474 — pregame at check time, first pitch 16:40 UTC):** owner pushed commit 7b904c4; navigated live Chrome to `sportstrata.cc/#mlb-live-824474` and confirmed against the actual rendered page — breadcrumb reads "MLB SCORES › STL @ CIN" (fix works, no more raw hash), the two-column sidebar renders with real data (Game Info: Great American Ball Park; Game Notes: Mound Visits Left STL 4 · CIN 4), the pregame badge shows the correct first-pitch time, and `_buildProbablePitchers` correctly lists both starters (Michael McGreevy vs. Brady Singer). No console errors.

**Bug found in that same check, fixed, not yet re-verified:** `document.querySelector('.lg-tabs')` showed `hidden: true` (attribute set correctly by the pregame branch) but `computedDisplay: "flex"` — the shared `.mlb-group-toggle-row` class's `display:flex` is author-origin CSS and beats the user-agent `[hidden]` rule regardless of specificity or source order, so the Play-by-Play/Box Score tab buttons stayed visible and clickable pregame; since `_switchTab()` doesn't check `isPreview`, clicking one would silently blank out the probable-pitchers content. Fixed with an explicit `.lg-tabs[hidden] { display: none; }` override, committed locally (commit `adaefe3`, parent `7b904c4`) — **not yet pushed**, so this specific fix has not itself been re-checked live. Owner needs to `git push` again before it's confirmed on production.

**Still unverified, no MLB game has been live at any check so far** (all checks landed pregame or off-hours — next first pitch for the game used above is 16:40 UTC today): base-diagram persistence mid-at-bat, the spin-rate tooltip content during a real live pitch, sidebar mound-visits/challenges updating on a live poll, and the full two-column layout under actual live 9s polling (only checked pregame, where polling is the slower 60s pregame cadence). These need a pass once a real game is in progress — flagging honestly rather than treating pregame verification as covering live behavior it doesn't exercise.

**Implications:** `sw.js` cache version was not bumped this pass (out of scope for a doc/code pass without a push) — whoever runs `/deploy-check` before the eventual push needs to confirm it picks up the `liveGame.js`/`liveGame.css` changes. Win-expectancy follow-up needs a new ISSUES.md entry once Relay's spec exists — not opened yet, this entry is the pointer. Trajectory animation needs its own Kael/Vera visual+behavioral spec before Finn builds it — same three-gate discipline as every other feature-level item in this project, not a quick add just because the data gate cleared.

---

## D-117 — Service worker was caching every /api/ response as a static asset: site-wide stale-data bug found and fixed (2026-08-22)

**Status:** complete (fix shipped locally, live-verification pending deploy)
**Contributors:** Finn (investigation), Axiom (root cause + fix)
**Date opened:** 2026-08-22 | **Date resolved:** 2026-08-22

**Decision needed:**
  None -- a bug fix, logged as a full entry rather than an ISSUES.md-only note because it's site-wide (every sport, not just NFL) and touches shared architecture (`sw.js`'s fetch handler), per ROUTER.md's tier-3 threshold.

**Context:** Continuing tonight's NFL live-game polish pass (see ISSUES.md's 2026-08-22 entries), opened a real, already-completed preseason game (SF @ LAC, event `401873285`, final 41-17 two days earlier) as a test case for the live game page's final-state rendering. The page rendered it as **pregame** -- "8/20 - 10:00 PM EDT", no scores, Play-by-Play/Analytics/Fantasy tabs all in their "not started yet" empty states -- for a game that had been over for two days.

**Root cause, confirmed live, not assumed:** `js/nflLiveGame.js`'s own fetch of `/api/nfl?path=/summary&event=401873285` was correct on every manual re-check (`STATUS_FINAL`, real scores) -- the bug wasn't in the NFL code or in `functions/api/nfl.js`'s own edge-cache logic (D-095's cache-key salting there is intact and working). The wrong data was coming from `sw.js`'s own fetch handler: its "stale-while-revalidate for all other same-origin assets (JS, CSS, images, fonts)" branch has **no path check at all** -- it only excludes non-GET requests, cross-origin requests, and full-page navigations. Every `/api/*` call across the entire site (NFL, MLB, NCAAF, NCAAB, WNBA, auth, prefs -- all of it, confirmed by grep: every fetch in every `js/*.js` file targets an `/api/...` path) falls through into the same bucket as static JS/CSS/images by omission, not by design -- the block's own comment has always said "JS, CSS, images, fonts" and never meant to include dynamic data.

Direct evidence, not inference: inspected the live Cache Storage in Chrome (`caches.open('sportstrata-v208')`) and found **84 `/api/` entries already sitting in the "static asset" cache** alongside real static files -- MLB `statsapi.mlb.com` proxy calls, NFL summary/scoreboard calls, all of it. Stale-while-revalidate means the *first* hit to any given `/api/...` URL after a long gap serves whatever's sitting in that cache immediately (however old) and only refreshes it in the background for the *next* visit -- exactly matching what was observed: the very first fetch of the SF@LAC summary URL returned the 2-day-old pregame payload, and every fetch after that (including a plain uncached JS `fetch()` moments later) returned the correct final data, because the background revalidation from that first hit had already updated the cache.

**Why this is bigger than D-095:** D-095 (2026-08-14) found and fixed staleness in the *edge* leg -- this Function's own fetch to ESPN, bounded to whatever window Cloudflare's best-effort TTL eviction allows (seconds to low minutes, and only for NFL). This bug is in the *client* leg -- the service worker's own Cache Storage, which never expires on its own (no TTL concept at all, unlike `functions/api/*.js`'s deliberate `Cache-Control: max-age` headers) and persists until `CACHE_NAME` bumps. It affects every sport's `/api/*` proxy, not just NFL, and the staleness window is unbounded (days, in the one caught here) rather than seconds.

**Decision:** Fixed `sw.js`'s fetch handler to route every `/api/*` request straight to the network, before the stale-while-revalidate branch -- letting the origin's own per-path TTL logic (`functions/api/nfl.js`'s `ttlFor()`, and the equivalent in `mlb.js`/`ncaaf.js`/`ncaab.js`/`wnba.js`) govern freshness the way it was always meant to, instead of being silently overridden by an indefinite SW cache. No fallback-to-cache on network failure for these requests -- matches this codebase's existing posture of failing loud through `ErrorHandler`/ each page's own try/catch rather than silently serving something wrong.

**Verified locally:** `node --check` clean on `sw.js`, 0 NUL bytes. Grepped every `fetch(\`/api/...\`)` call site across `js/*.js` to confirm the `/api/` prefix covers all of them (NFL, NCAAF, NCAAB, WNBA, MLB's `mlbFetch`, Sleeper, auth, news) -- no dynamic-data endpoint lives outside that prefix, so the new guard doesn't need a second pattern. `sw.js` was already being bumped v208 to v209 this same session for the earlier Game Leaders sidebar fix (see ISSUES.md) -- this fix rides the same version bump, so the buggy v208 cache (with its 84 stale entries) gets discarded on activate regardless.

**Not live-verified the way client-JS fixes normally are** -- this is a service-worker file; the fix only takes effect once deployed and a client's SW re-installs (browsers check for SW byte-diffs on navigation). Root cause and the fix's correctness were confirmed as thoroughly as possible pre-deploy: the stale-cache evidence above is a direct Cache Storage inspection (not inferred), and the fix logic (route `/api/*` around the SWR block) was checked against every actual API call site in the codebase rather than assumed to be complete.

**Rationale:**
  The service worker was never meant to cache dynamic data -- its own comments say so -- so this is a real bug, not a design tradeoff to weigh. Routing `/api/*` straight to network is the narrowest fix that restores the origin's intended TTL behavior without touching any of the per-sport caching logic (`functions/api/*.js`, `js/cache.js`'s `ApiCache`) that already does the right thing when the SW isn't shadowing it.

**Implications:**
  Next live NFL (or any-sport) window after deploy should spot-check a just-finished or just-started game's page on a fresh browser profile / after clearing site data, to confirm no stale pregame/final state renders on first visit. If any other same-origin dynamic endpoint is added outside `/api/` in the future, it needs the same treatment -- flagging this pattern for whoever adds the next Pages Function route. `functions/api/mlb.js`, `ncaaf.js`, `ncaab.js`, `wnba.js` all use the identical `cf.cacheTtl`/`cacheEverything` upstream-caching pattern D-095 already noted as shared exposure for the *edge* leg; this fix closes the *client* leg for all of them at once since it's keyed on `/api/` generally, not per-sport.

## D-118 — NCAAF live game viewer: Phase 1 skeleton shipped, six-tab production build deliberately not attempted yet (2026-08-22)

**Status:** complete for Phase 1 scope (shipped locally, pending owner push); deeper tabs explicitly deferred, not scheduled
**Contributors:** Axiom (implementation + scoping), Finn (live verification)
**Date opened:** 2026-08-22 | **Date resolved:** 2026-08-22

**Decision needed:**
  None for what shipped -- logged as a full entry rather than an ISSUES.md-only note because this is new architecture (a new file, a new view/routing pattern, a new frontend consumer of an already-allowlisted-but-unused API path), per ROUTER.md's tier-3 threshold, and because it deliberately mirrors D-080's own phased-scoping precedent for NFL.

**Context:** owner asked to "start to build out the skeleton to implement ncaaf live scores like we are on nfl." NCAAF (D-042) shipped Scores/Standings/Teams/Rankings but never had a live-game-detail view -- confirmed live before building anything: `_ncaafGameCard()` (`js/ncaaf.js`) rendered every game as a static, non-interactive div with no `onclick`/`role="button"` at all, and `_renderNCAAFView`'s dispatch had no `ncaaf-game-` case. Real finding, not assumed: `functions/api/ncaaf.js`'s path allowlist already includes `/summary` with a 20s live-game TTL and an explanatory comment ("live game detail") -- cloned wholesale from `nfl.js`'s own allowlist at D-042 time, but no frontend code had ever called it. The backend half of this feature was already sitting there unused.

**Decision: this ships as a Phase 1 skeleton, not NFL's six-tab production build, for the same reason D-080 itself phased NFL's own rebuild.** D-080 named one explicit open risk before Finn built NFL's Play-by-Play tab: whether `/summary` actually carries a populated `drives`/`plays` array couldn't be confirmed without a live in-progress game, and that risk was only cleared later, live, against a real game (D-105/D-106). The identical risk exists here, and live-checking it tonight ruled out one path forward, not both: fetched the real `/api/ncaaf?path=/scoreboard` and confirmed the 2026 NCAAF season has not started (earliest scheduled game: Aug 29, SJSU @ USC, event `401864494`) -- there is no in-progress NCAAF game anywhere to check `drives`/`plays`/`scoringPlays` against right now. Building a Play-by-Play or Box Score tab on an unverified assumption of that shape would repeat exactly the mistake this project's own house rules exist to prevent (see D-080's own framing, and the nflverse Phase 3b gate this same session re-checked).

**What shipped:** `js/ncaafLiveGame.js` -- `showNCAAFGame(eventId)` fetches `/api/ncaaf?path=/summary&event={id}` and renders a score header (team logos/records/AP rank/score, live-status badge) plus a venue/broadcast info row, followed by an explicit, honest placeholder explaining what's deferred and why -- not a blank shell, not a faked box score. `js/ncaaf.js`: `_renderNCAAFView` now dispatches `ncaaf-game-{id}` to `showNCAAFGame`; `_ncaafGameCard()`'s cards are now real click targets (`role="button" tabindex="0" onclick="navigateTo(...)"`, matching the same accessible-link pattern this file already uses for team chips and leader rows -- no new interaction pattern invented). A lightweight 20s poll re-fetches and re-renders the header while a game is live, matching `functions/api/ncaaf.js`'s own `ttlFor('/summary')` cadence -- polling faster would just re-serve the same edge-cached response, same reasoning D-080 used for NFL.

**Zero new CSS.** Reused `.nlg-*` header classes from `css/nflLiveGame.css` (already loaded unconditionally on every page load, confirmed via `index.html`'s `<link>` list -- CSS classes aren't JS-scoped, and this codebase already has precedent for cross-sport class reuse: `ncaaf.js` itself already reuses NFL's own `.nfl-offseason`/`.nfl-offseason-text` empty-state classes verbatim), `.player-bio-grid`/`.player-bio-item` (`css/components.css`, `detailFrame.js`'s own cross-sport bio-chip pattern) for the venue/broadcast row, and `.nfl-offseason`/`.nfl-offseason-text` again for the deferred-tabs placeholder.

**Explicitly deferred, named rather than silently skipped:** the six-tab structure (Play-by-Play/Box Score/Team Stats/Analytics), live in-progress situation rendering (down/distance/possession -- NCAAF has no equivalent of NFL's `/scoreboard`-sourced field-viewer situation fetch built yet either), win probability, and any Highlight-Card-Studio tie-in. None of these are scheduled. Revisit once a real NCAAF game is live -- kickoff week, Aug 29 and after -- and `/summary` can be checked against real in-progress data the same way D-105/D-106 checked NFL's, rather than assumed from this session's pregame-only sample.

**Live-verified before committing, not assumed from the diff:** monkey-patched every new/changed function into the running production page. Called `showNCAAFGame('401864494')` directly -- header rendered real team logos/colors/records, correct "8/29 - 3:00 PM EDT" scheduled status, venue chip ("Los Angeles Memorial Coliseum — Los Angeles, CA"), and the deferred-tabs placeholder, with no layout artifacts (confirmed `#playersGrid`'s className was correctly reset to `player-detail-container`, not left at the `players-grid` card-grid class that crushed Highlight Card Studio's layout earlier this same session -- same lesson applied proactively here rather than found as a second bug). Then verified the real click path end-to-end: patched `_ncaafGameCard`/`_renderNCAAFView` into the live page, navigated to the real College Scores grid, clicked the actual SJSU @ USC card, and confirmed it routed to `#ncaaf-game-401864494` and rendered identically, with the Back to Scores round-trip working. `tools/check-manifest.cjs` clean (index.html/sw.js/disk all in sync), full test suite 33/33, `node --check` + 0-NUL scan clean on all four touched files (`js/ncaafLiveGame.js` new, `js/ncaaf.js`, `index.html`, `sw.js`). `sw.js` bumped v210 -> v211 (new file is precached).

**Rationale:**
  The backend half of this feature (the `/summary` allowlist entry) had been sitting unused since D-042 -- building the frontend consumer was low-risk and high-value on its own, but the deeper tabs carry the same real, unverified-data-shape risk D-080 already named once for NFL and got right by waiting for live confirmation. Phasing this the same way is consistency, not caution for its own sake -- D-080's own precedent already proved out.

**Implications:**
  Once the 2026 NCAAF season starts (Aug 29+), the next live-verification pass on a real in-progress game should check `/summary`'s `drives`/`plays`/`scoringPlays` fields the way D-105/D-106 checked NFL's, before scoping the six-tab build further. No CLAUDE.md architecture disruption beyond a new Key Files row and script-chain entry (doc-sync pass to follow in the same session). No new external host, no CSP change -- `functions/api/ncaaf.js`'s `/summary` path was already allowlisted and same-origin.
