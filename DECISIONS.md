...
**Verified locally:** `node --check` clean on `js/app.js` + `js/nflLiveGame.js`, 0 NUL bytes, 33/33 unit tests. The drive-dedup fix (Finding 2) was live-verified against real, currently-changing ESPN data before shipping (see above) — the strongest verification standard used this session. The poller fix (Finding 1) was verified by code/logic review rather than a full 5-minute live-timing wait: `_lastFetchAt` starts at `0`, so `Date.now() - 0` is trivially `>= FORCE_REFRESH_MS` on the very first tick after page load, guaranteeing an initial fetch always happens; after that, the guard only ever skips for at most 5 minutes before forcing a recheck. This is a lower verification bar than Finding 2's — flagged honestly rather than claimed as fully live-tested. `sw.js` bumped for this ship — see the commit for the exact version.
