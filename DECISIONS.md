...
## D-079 — Push Notifications v1 implemented: game-start alerts for followed teams (F5)

**Status:** implemented, not yet deployed (owner action required — no wrangler/Cloudflare auth available from this environment)
**Contributors:** Vera (behavioral spec), Axiom (feasibility + implementation), Kael (visual pass-through, confirmed no new component needed)
**Date opened:** 2026-08-09 | **Date resolved:** 2026-08-09 (code); deployment pending

**What shipped**, per the three-gate spec already recorded in ISSUES.md ("Push Notifications — Game-Start Alerts for Favorited Teams (F5) — Three Gates"): a new D1 migration (`push_subscriptions` + `push_sent_log`), a session-scoped Pages Function (`functions/api/pushSubscribe.js`), a cron Worker (`worker/push-game-alerts.js`, every 10 minutes) that checks MLB/NFL schedules for games starting soon and sends Web Push via `@block65/webcrypto-web-push`, two new `sw.js` listeners (`push`, `notificationclick`), and a settings-panel toggle in `js/auth.js` next to the existing weekly-digest opt-in. Signed-in required, free (not paid-entitlement-gated) — consistent with D-034/D-031's additive-only rule, since this is a new capability, not a gate on anything already free.
...
**Real technical risk, named and not hidden:** `@block65/webcrypto-web-push` is a small (3-dependency) package, not a widely-battle-tested one like the Node-only `web-push`. The MLB team-id→abbreviation map and ESPN status-code strings in `push-game-alerts.js` were written from documented API shape, not a live response — this sandbox has no outbound route to `statsapi.mlb.com` or ESPN to verify against (confirmed earlier this session; only the npm registry is reachable). **The cron trigger should not be trusted blind** — run one `/__run` hit during a real pre-game window before relying on it, same spike-before-commit discipline already applied to html2canvas and the Highlight Card Studio's crypto-adjacent risk items.

**What the owner still needs to do (listed in full in ISSUES.md's Implementation addendum):** `npm install` at root, apply migration 0007, set three Worker secrets (`VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY` — the real generated key pair was shared in this session's chat, not committed to source — and `PUSH_RUN_SECRET`), deploy the Worker, then run the pre-enable spike. None of this is optional-later cleanup; the feature does nothing in production until these steps happen.
