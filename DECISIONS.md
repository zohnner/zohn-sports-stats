...
## D-031 — Accounts, retention & monetization foundation — Phase 1 LIVE-VERIFIED + security-reviewed 2026-08-04 (one open finding, owner decision needed)
...
**Not done yet:** (a) decide on the `/__run` finding above, (b) confirm `nodejs_compat` is set in the Cloudflare Pages dashboard itself (Production + Preview — the local `wrangler.toml` flag doesn't cover the dashboard-built live site), (c) wire follow stars onto the existing card templates (deferred, ISSUES.md task).
