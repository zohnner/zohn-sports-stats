...
- **S2 — Scorebug unification (M/L, in progress).** `js/scorebug.js` — `renderScoreCard`/`renderTickerItem` + `normalizeMLBGame`; faithful extraction of the current `.home-game-card`/`.ticker__item` anatomy so migrations are drop-ins. **Shipped:** foundation (`f0af0e1`, unit-tested, `tests/scorebug.test.js`) + **consumer 1** — the home grid `_gameCard` now delegates to `Scorebug.renderScoreCard(normalizeMLBGame(g), {favStar})`; the old inline card + live helpers removed. Next consumers, one commit each with a live check: MLB ticker (`updateMLBTicker`) → NFL → NCAAF → NHL/NBA. Sport-agnostic ticker schema (deferred D-046 P6b) folds in here. Excludes the Expanded View.
