...
## D-031 — Accounts, retention & monetization foundation — Phase 1 IMPLEMENTED 2026-08-04 (pending push + owner live-verification)
...
**Not done yet:** the owner still needs to (a) fill in `AUTH_TURNSTILE_SITE_KEY` in `js/auth.js` — sign-in cannot succeed against the Turnstile-gated endpoints without it, (b) deploy the purge Worker separately, and (c) run the spike acceptance checklist for real via `wrangler pages dev` — this is what actually proves the two disclosed-unverified items above one way or the other. A full `/security-review` gates this before it goes anywhere near production traffic.
