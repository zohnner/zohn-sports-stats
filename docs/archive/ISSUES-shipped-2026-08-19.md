# ISSUES Archive — shipped/historical, moved 2026-08-19

Second archival pass (first was 2026-07-26, see ISSUES-shipped.md). Moved from the live ISSUES.md to cut session-start token cost — the live file was ~517KB/3,126 lines, most of it resolved/historical. Sections below are unedited, moved verbatim. See ISSUES.md's own "Archived — shipped / historical (moved 2026-08-19)" stub for the index.
...
**Update 2026-08-09 — owner reported "sportstrata" still doesn't surface in search; investigated, found two separate causes.** (1) A real, unrelated, established business already owns the bare term — SportStrata, a sports psychology coaching firm (sportstrata.com, founded 2016, with its own LinkedIn/Instagram/Facebook/Yelp/ZoomInfo presence). No technical SEO on sportstrata.cc changes that. Owner decision: don't chase the bare term, target qualified phrases instead ("sportstrata mlb stats," "sportstrata nfl," etc. — realistic to win, and how people actually search once they know the site from a link or share). (2) Separately, live search checks today (`site:sportstrata.cc`, "sportstrata.cc mlb stats", "sportstrata sports analytics baseball stats") surfaced zero pages from the domain at all — consistent with D-056's fetch-fix being only a week old on a domain with no backlink history yet; likely still an indexing/authority lag, not a new bug, but not confirmed against real Search Console data (owner-only access).

**Concrete bug found and fixed while auditing for the qualified-term direction:** `functions/nfl/index.js`'s crawlable-snapshot `CARDS` linked to `/mock-draft` and `/draft-kit` — neither is a real route. `_routes.json` only routes `/nfl` and `/nfl/*`; the actual views are hash-routed `#nfl-mock`/`#nfl-draftkit`, same as the array's own other two entries (`#nfl-standings`, `#nfl-games`). Two of the four links on the NFL landing page — the page most likely to rank for "sportstrata nfl"-style qualified queries — were broken, for both crawlers and real visitors. Checked MLB's and NCAAF's equivalent landing files for the same pattern: both clean, all links valid, bug was isolated to NFL. Fixed to `#nfl-mock`/`#nfl-draftkit`. The rest of the on-page foundation checked out well already — MLB/NFL/NCAAF landing titles and descriptions are specific and content-matched (not generic, not keyword-stuffed), `/mlb/leaders` is purpose-built for high-volume evergreen queries ("home run leaders," "ERA leaders"), JSON-LD is present and reasonable throughout.
...
**Contributors:** Vera (behavioral), Kael (visual), Axiom (feasibility) | **Date:** 2026-08-08

**Context:** Full-team audit (docs/full-audit-2026-08-08.md) and the resulting roadmap (docs/roadmap-2026-08-08.md) found that AI League Insights, Personalized Fantasy Grade, and Weekly Fantasy Digest all have three-gate specs (two "implementing this pass," one landed-not-implemented) — but every one of them terminates at the same stub: `functions/api/_entitlement.js`'s `isEntitled()` unconditionally returns `false` because `subscriptions` has zero rows for anyone. There is no Stripe code anywhere in the repo — no Checkout call, no webhook handler, no `STRIPE_*` secret. Per this file's own three-gate rule, this needs its own spec before implementation starts, same as any other feature — it's been treated as an implicit prerequisite rather than a scoped item until now.

**Job to be done (Vera):** "I've decided I want the paid tier. Let me pay in a way I already trust (Stripe's own hosted page, never this site's own form), and have it just work the next time I load an Insights/Grade view — no manual step, no support ticket to 'activate' anything." The job is specifically **invisible plumbing** — the user-facing job to be done was already specified inside each of the three feature entries (their own "upgrade" CTAs and free/paid states); this entry's job is making those CTAs actually do something.

**Behavioral spec (Vera):** One upgrade entry point, reused by all three features rather than three separate purchase flows — clicking any "Upgrade" CTA (already speced in each feature's own entry) opens Stripe Checkout (hosted, redirect-based) with SportStrata's return URL bringing the user back to whichever page they started from. States: pre-purchase (existing free-tier states, unchanged), redirected-to-Checkout (no SportStrata-side loading state needed — it's a real page navigation), post-purchase-success (Checkout's own success redirect lands back on the originating view; the paid state should reflect immediately on that load, not require a manual refresh or a delay), post-purchase-webhook-lag (real risk: Stripe's webhook can arrive after the success-redirect page load — the return page must not show a false "still free" flash; either poll briefly or trust the Checkout session id client-side as an optimistic signal until the webhook confirms, Axiom to specify which), payment-failed/abandoned (Checkout's own cancel URL returns to the originating page with no change — no error state needed on SportStrata's side, Stripe owns that UX), and subscription-lapsed (an existing paid user whose `current_period_end` passes reverts to the exact free-tier states already speced per feature — no separate "your subscription expired" interstitial, the existing free states already communicate the boundary honestly).

**Visual spec (Kael):** No new visual surface beyond the "Upgrade" CTA already speced identically in each of the three feature entries (same visual weight as the existing sign-in trigger buttons, never a modal interruption). Stripe Checkout itself is Stripe's own hosted, unstyled-by-us page — no design work owed there. The one new element: a small, honest confirmation state on return from a successful purchase (a brief, dismissable "You're on the paid plan" line reusing `.settings-account-row`'s established text-plus-action shape) — not a celebratory modal or confetti, consistent with this product's stated no-gamification posture (DESIGN.md).

**Feasibility (Axiom):** Stripe Checkout (hosted page — SportStrata's code never touches card data, satisfying the standing "never handle payment credentials" rule already named in each feature's own spec). New endpoint `functions/api/stripe/checkout.js` creates a Checkout Session server-side (never trusts a client-supplied price) and redirects. New endpoint `functions/api/stripe/webhook.js`, signature-verified via Stripe's signing secret, is the only writer to the `subscriptions` table (upserts `status`/`current_period_end` on `checkout.session.completed`, `customer.subscription.updated`, and `customer.subscription.deleted` events) — mirrors the "server is authoritative, never trust a client claim" pattern already enforced on every D-031/D-064/D-065/D-069 endpoint, extended to payments rather than reinvented. Webhook-lag handling (the one real new UX risk, flagged in the behavioral spec above): recommend the return page reads the Checkout Session id from the redirect URL and does one short-poll (a few seconds, a few attempts) against `isEntitled()` before falling back to the existing free-tier state rather than either blocking indefinitely or flashing a false negative. Requires new secrets (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`) via `wrangler secret put` — never in source, same discipline as every existing secret in this project. **Blocked on:** the owner's pricing decision (docs/roadmap-2026-08-08.md proposes $4.99/mo or $29/yr as a starting number) and a real Stripe account/product setup, both owner actions this spec doesn't do for them.

**Gate status: visual and behavioral specs present. Feasibility present with one explicit open implementation question (webhook-lag handling approach) for Axiom to finalize during the build pass, not a blocking gap.** Not yet implemented — ready for a build pass once the owner confirms pricing and creates the Stripe product/prices. This is the one concrete blocker standing between the three already-spec'd paid features and actually generating revenue.
...
**Home hero — shipped 2026-08-09 (owner ruling via two-question clarification, full detail in DECISIONS.md D-077).** Calendar-driven, hero-copy-only: `_homeHeroSport()` picks MLB for months 3–10, NFL for 11–2 (same boundary `MLB_SEASON` already uses), `_renderHomeHero()` branches to a new `_renderHomeHeroNFL()` in the NFL window. D-042's neutral home brand, sport-picker band, and every other home section stay untouched and MLB — only the hero's own headline/kicker/game content changes. Caught and fixed a real latent bug while building it: `showNFLGame()` never self-set `AppState.currentView` (safe until now only because every prior caller went through `navigateTo()` first) — the hero's click handler is the first direct caller, so it got the same D-075-pattern fix `showMLBGameDetail()` already received. Not yet live-verified against the real NFL branch — today's date sits inside the MLB window, so verification needs a forced console call, not a normal page visit.
...
**Everything below is committed AND pushed AND live-verified. Last commit: `7b0bcda` (NCAAB season-label fix). SW at v163. Nothing pending push.** Owner is starting a fresh Cowork chat; this is what that chat needs to pick up cleanly. DECISIONS.md/GOALS.md/ISSUES.md are already current as of this entry — a new session gets full context just from reading those per the standing team protocol, but the summary below is the fast-path version.

### Shipped + live-verified this session (all decision-logged)
- **CSP sync fix:** `index.html`'s CSP meta tag was missing `i.ytimg.com` that `_headers` had (from D-085's YouTube thumbnails) — broke CI check #3. Fixed, also corrected D-085's own stale note that wrongly said not to touch `index.html` for this. Commit `2533a78`.
- **D-086 — Draft Instincts (videocreation pipeline) build-out paused.** Owner got 50,000 views in one day from manual clip curation + SEO metadata, using none of the built video-automation pipeline, which after a week of engineering had produced zero videos with confirmed audience data. Team recommendation: pause further pipeline feature work (not decommission — it's sunk-cost-built, costs nothing idle), redirect effort to the workflow that actually worked, use `youtube-insights.html` as the feedback loop. Commit `a24cb5a`.
- **BRAND-KIT.md** — prompt-ready brand reference (colors, fonts, logo paths, existing intro/outro visual precedent, a copy-paste prompt base) for prompting external AI image/video tools, per D-086's redirect. Commit `f5ce008`.
- **D-052 — NCAAB (Men's College Basketball) as a 4th sport.** Owner asked to reconsider adding NBA; team check found D-052 (open since 2026-07-31, never ratified) had already compared NBA revival against NCAAB and recommended NCAAB for the Dec–March calendar gap NBA doesn't fill (still true — no NBA registry entry, no NBA proxy, BDL `/stats` still 401 on the free tier). Owner ratified NCAAB. Built and shipped in phases:
  - P1 — `SPORTS` registry entry (pure enabler, held out of the visible list at first). Commit `3ecb995`.
  - P2/P3 — `functions/api/ncaab.js` + `functions/api/ncaabstandings.js` (ESPN proxy clones, same pattern as NCAAF) + `js/ncaab.js` (Scores/Standings/Teams/Rankings), wired into nav, added to the visible `SPORTS` list. Commit `446469d`.
  - **Live-verify caught a real bug:** the season-label constant only flipped at November instead of right after the previous season ends in April, so the entire May–Oct offseason showed the wrong season number in the "tips off in November" copy — AND the same constant was wrongly driving the Standings/Teams default selector, which needed the opposite behavior (keep showing last season's real data, not jump to an empty upcoming one). Fixed by splitting into `NCAAB_SEASON` (copy) and `NCAAB_LAST_SEASON` (data default), mirroring the exact split `ncaaf.js` already uses for this same tension. Commit `7b0bcda`.
  - Re-verified live on production after the fix: sport switcher, landing page, Scores offseason copy (now correct), Standings/Teams/Rankings (still correctly default to 2025-26 season with real final data) — all confirmed, zero console errors.
  - **P4 (player leaders/detail) data-checked, not built.** Probed ESPN's core API live: team rosters return strong headshot coverage (13/14 on the one team checked) plus full bio; the leaders endpoint returns real per-athlete stats (points/assists/rebounds/steals/blocks/FT%/FG% per game), same ID-ref shape NCAAF's turned out to have under D-044. Data supports lifting the deferral, same signal that flipped NCAAF's. **Not built — owner hadn't answered go/no-go when this session ended**, see Ratified queue below.

### Ratified queue (in order — pick up here)
1. **NCAAB P4 decision, unresolved.** Asked the owner "build player leaders/detail now?" and got redirected to this handoff instead of an answer — so **the very first thing the new chat should do is ask this again** (or the owner states it directly). If yes: mirror NCAAF's own D-044 P2/P3 exactly — `functions/api/ncaabathlete.js` (clone of `ncaafathlete.js`, ID-resolved roster+stats, no name-matching), a Leaders view (category cards from the probed leaders endpoint), player detail on the shared `detailFrame.js` builder. Template code for all of this already exists in `js/ncaaf.js`'s equivalent sections — this is a clone-and-adapt job, not a design-from-scratch one.
2. Everything else in DECISIONS.md's "Open, owner-gated" line (D-025, D-043, D-053 pt. 2, D-031) is unchanged from before this session — none of it was touched.

### Owner to-dos (nobody else can)
- None outstanding — every commit this session was pushed and live-verified before handoff.

### Still owed (small)
- Task #30 in the running task list (Highlight Card Studio live-verify) has been pending since a much earlier session — not touched this session, still open if anyone picks it up.

### Working conventions for the next session (unchanged, restated for a cold start)
- **Git is broken on this mount** — `git status`/`git add`/normal commits don't work reliably. Use the plumbing workaround: temp `GIT_INDEX_FILE`, `git read-tree HEAD`, `git add` only the touched files, `git write-tree`, `git commit-tree` with a heredoc message, `echo "$NEW" > .git/refs/heads/main`. "unable to unlink tmp_obj" warnings during this are cosmetic. Verify via `git diff --stat HEAD~1 HEAD` + per-file `diff -q <(git show HEAD:"$f") "$f"`, not `git status`.
- Bump `sw.js`'s `CACHE_NAME` on every JS/CSS change (currently v163) — it's what pushes precached assets past an already-installed service worker on returning visitors.
- After every commit: confirm the user has pushed, then live-verify via the Claude-in-Chrome tools (`sw.js` version check first to confirm the deploy landed — Cloudflare Pages builds took 60–100 seconds this session — then screenshot/console/network checks on the actual feature). Deploys are NOT instant; don't conclude a bug from a stale render without confirming the SW version first, and don't trust a single render if a service worker might still be serving a cached asset — a `location.reload()` resolved that exact false alarm this session.
- Three-gate rule holds: don't start Finn-level implementation on a new feature until Vera/Kael/Axiom (and Relay if data's involved) have real, specific findings — not just an assumption the pattern from a sibling sport will hold. The NCAAB P4 data check this session is the model: probe live before promising scope, the same discipline that already caught NCAAF's own standings-feed-is-a-stub surprise once.
- DECISIONS.md/ISSUES.md/GOALS.md doc-sync rule: any decision that ships touches these in the same commit, not as a follow-up.
...
**Context:** owner directly requested WNBA after D-052 (same day) had evaluated and passed on it in favor of NCAAB, which shipped earlier this session. The conflict was surfaced to the owner before any code was written; owner chose to proceed with WNBA anyway. Full reasoning, live-verified data findings, and the explicit "known trade-off, not resolved" note are in DECISIONS.md D-092 — this entry mirrors the gate outputs so Finn-level implementation has a documented spec, per standing protocol.

**Scope:** Scores, Standings (Eastern/Western conference-grouped), Teams. No Rankings view (WNBA has no poll). Player leaders/detail deferred, same posture as NCAAF/NCAAB's original deferral.

**Vera (JTBD/states):** three jobs — live/today's scores, conference standings, team browse. Offseason (Nov–Mar) state names the threshold ("starts April") with MLB-live + back-to-home actions, mirroring `_ncaabOffseasonState()`. Loading = skeleton shimmer. Error states name what failed + retry, no "Something went wrong." Cleared.

**Kael (visual):** zero new visual language — reuses `.standings-*`, `.hgc-*`/`.home-game-card`, `.nfl-offseason`, `.ncaaf-team-grid`/`.ncaaf-team-chip` verbatim, same as NCAAB. No new CSS file. Cleared.

**Axiom (feasibility):** direct clone of `functions/api/ncaab.js` → `wnba.js`, `functions/api/ncaabstandings.js` → `wnbastandings.js`, `js/ncaab.js` → `js/wnba.js`. One registry entry in `SPORTS_META`/`SPORTS` (`js/navigation.js`), added directly to the visible list (no phased hold-back needed — data layer ships in the same pass, unlike NCAAB's P1/P2/P3 split). **Real gap found while tracing the clone target, not fixed here (out of scope):** `js/ncaab.js`'s ticker calls `Scorebug.normalizeNCAAFGame`, which hardcodes `sport:'ncaaf'` (`js/scorebug.js` line 124) — NCAAB's ticker items have carried the wrong league glyph and `data-sport` value since D-052 shipped. This is the specific mechanism behind the gap already flagged at the bottom of the "Home — Cross-sport score ticker" entry above ("NCAAB has no `Scorebug.normalize*Game` yet"). WNBA's own ticker function is self-contained and does not call `normalizeNCAAFGame`, so it does not inherit this bug — but NCAAB's existing bug is untouched, flagged again here for whoever picks it up. Cleared.

**Relay (data contract) — live-verified 2026-08-10 before any code written:** `site.web.api.espn.com/apis/site/v2/sports/basketball/wnba/scoreboard` and `/teams` return real, populated, in-season data (confirmed against a live in-progress game). `site.web.api.espn.com/apis/v2/sports/basketball/wnba/standings` returns a flat two-conference tree (Eastern: 7 teams, Western: 8 teams, no division nesting) — simpler than NCAAB's recursive conference tree; same collector function handles it. 14+ teams confirmed live including both 2025/2026 expansion franchises (Golden State Valkyries, Toronto Tempo) — nothing hardcoded, so roster/franchise churn is a live-data concern only. Season is a single calendar year (unlike NCAAF/NCAAB's fall-spanning label) — real window confirmed 2026-04-03 through 2026-10-20. Cleared.
...
**Resolution 6 (2026-08-12) — live game view + Playoff Picture, owner request to bring WNBA to parity with MLB/NFL/NCAAF/NCAAB ("live score views, player stat insights, comprehensive playoff bracket outlook").** Player stat insights: already covered by Resolution 5, no new work. Two new surfaces, each gated by a live check first:

- **Live/Final Game panel.** Relay fetched a real `/summary?event=...` response before any code: unlike NFL's summary, WNBA's has no top-level score/status header — `boxscore.teams[].statistics`/`leaders[]` are season averages, not this-game's box score. Decision: score/status comes from the existing scoreboard fetch (already used by the Scores view); `/summary` supplies season-context stats/venue/broadcasts, explicitly labeled "season" in the UI. Cloning NFL's 6-tab live dashboard was rejected as disproportionate to what WNBA's summary endpoint actually returns — built a single non-tabbed panel instead. Shipped `showWNBAGame(id)` / `wnba-game-{id}` in `js/wnba.js`, wired into `_wnbaGameCard` onclick, the shared ticker's click handler (new `wnba` branch in `setupTickerClicks()`, `js/app.js`), and a `wnba-game-` hash matcher in `navigation.js` (mirrors `nflGameMatch`). Zero new CSS.
- **Playoff Picture.** Relay confirmed via WebSearch (ESPN's own bracket resource has nothing yet, playoffs start Sept 27) the real format: top 8 of 15 by overall record, no per-conference bracket, seeded 1-8. Decision: standings-derived snapshot only — not a Monte Carlo odds model (MLB's October Odds is a materially larger separate build) and not a fabricated bracket. Shipped `displayWNBAPlayoffPicture()` / `wnba-playoffs` — flattens both conferences into one league-wide ranked list, marks 1-8 "IN," computes games-back-of-8th-seed for 9-15, framed as "if the season ended today" with the real Sept 27 start date stated alongside. Wired into sub-nav, menu panel, `_PAGE_META`, SEO title map, hash-load allowlist — not bottom nav (mirrors NCAAB's own choice not to crowd Rankings there). Zero new CSS (reuses NCAAF Rankings' `.standings-rank`/`.standings-split` table pattern).
