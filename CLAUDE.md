...
**Script load order matters** (see `index.html`): `config.js` → `detailFrame.js` → `achievements.js` → `errorHandler.js` → `cache.js` → `schema.js` → `api.js` → `glossary.js` → `players.js` → `leaderboards.js` → `teams.js` → `games.js` → `charts.js` → `playerDetail.js` → `statBuilder.js` → `mlb.js` → `odds.js` → `scorecard.js` → `liveGame.js` → `shareCard.js` → `highlightCard.js` → `nfl.js` → `nflLiveGame.js` → `nflStandings.js` → `fantasy.js` → `nflPickem.js` → `sos.js` → `nhl.js` → `ncaaf.js` → `ncaafLiveGame.js` → `ncaab.js` → `wnba.js` → `bballLiveGame.js` → `arcade.js` → `standings.js` → `db.js` → `query.js` → `search.js` → `navigation.js` → `auth.js` → `news.js` → `scorebug.js` → `app.js`. Each file can reference globals defined by files loaded before it. **(Corrected 2026-09-09 — `nflPickem.js` added; `achievements.js` was also missing from this line despite being in `index.html` since before the prior correction, now fixed too. Verify against `index.html` directly if it drifts again.)**
...
| `js/bballLiveGame.js` | Shared basketball live-game dashboard (added 2026-09-08) — one tabbed viewer (Summary/Play-by-Play/Box Score/Team Stats/Analytics + a Win Probability/Game Leaders/Game Flow/Standings sidebar) parameterized by sport via `_BLG_SPORTS`, used by both WNBA and NCAAB (`_blgShow('wnba'\|'ncaab', id)`). Built after live-fetching real completed games from both sports' `/summary` endpoints and finding D-092 Resolution 6's original WNBA data-shape claims (no score header block, empty `leaders[]`) were wrong on a fresh check — both sports' `/summary` carry a full header/leaders/plays/winprobability/standings shape close enough to NFL's that one module covers both instead of two near-duplicate files (see the file's own header comment for the full inventory). Reuses `css/nflLiveGame.css`'s `.nlg-*`/`.gv-*` selectors verbatim, same choice NCAAF's viewer made from NFL's — no new stylesheet, no field-viewer (neither sport has spatial live data). No Fantasy tab (no fantasy feature for either sport); no clickable team blocks in the header (neither sport has a team-detail page yet). NBA (preview-only) has no entry in `_BLG_SPORTS` — not wired, per the standing "don't build NBA feature work unprompted" rule, but adding it later is one config entry, not new viewer work |
...
- `_renderSportSwitch(sport)` renders the header `.sport-switch` buttons from `SPORTS`; `index.html`'s static two-button (MLB/NFL) markup is only what a client sees before JS executes. **Mobile rework (2026-08-29):** below 768px, `SPORTS` now holding 5 entries no longer fits as an inline horizontal-scroll strip in the cramped header-inner row (it was the item most likely to silently overflow past the visible header, per that date's fix history in `css/main.css`). `.sport-switch-wrap` holds both a `.sport-switch-trigger` button (`#sportSwitchTrigger`, visible only <768px — icon + active sport's label + caret, kept in sync by `_renderSportSwitch`'s tail via `#sportSwitchTriggerIcon`/`#sportSwitchTriggerLabel`) and the full `.sport-switch` pill row (desktop: always visible inline; mobile: hidden until the trigger opens it as a dropdown). `_initSportSwitchMenu()`/`_openSportSwitchMenu()`/`_closeSportSwitchMenu()` (`js/navigation.js`) wire the same trigger+dropdown+outside-click+Escape pattern already used by `initMenu()`/`.auth-control`.
