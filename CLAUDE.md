...
**Script load order matters** (see `index.html`): `config.js` → `detailFrame.js` → `achievements.js` → `errorHandler.js` → `cache.js` → `schema.js` → `api.js` → `glossary.js` → `players.js` → `leaderboards.js` → `teams.js` → `games.js` → `charts.js` → `playerDetail.js` → `statBuilder.js` → `mlb.js` → `odds.js` → `scorecard.js` → `liveGame.js` → `shareCard.js` → `highlightCard.js` → `nfl.js` → `nflLiveGame.js` → `nflStandings.js` → `fantasy.js` → `nflPickem.js` → `sos.js` → `nhl.js` → `ncaaf.js` → `ncaafLiveGame.js` → `ncaab.js` → `wnba.js` → `nba.js` → `bballLiveGame.js` → `arcade.js` → `standings.js` → `db.js` → `query.js` → `search.js` → `navigation.js` → `auth.js` → `news.js` → `scorebug.js` → `app.js`. Each file can reference globals defined by files loaded before it. **(Corrected 2026-09-14 — `nba.js` added, D-161. `players.js`/`leaderboards.js`/`teams.js`/`games.js`/`playerDetail.js` are the pre-registry NBA-era files and are now dead code (nothing routes to their bare-name views any more) — left in the load chain rather than removed this pass; see DECISIONS.md D-161 continuation. Verify against `index.html` directly if this drifts again.)**
...
- All other views (including `'home'`, and the legacy pre-registry bare-name NBA views `'players'`/`'teams'`/`'games'`/`'standings'`/`'builder'`, now dead code) → shared switch statement
(Corrected 2026-09-14, D-161 — `nba-` dispatch added ahead of the legacy fallback switch. Corrected 2026-08-15 for the same reason on `ncaab-`/`wnba-`; verify against `js/navigation.js`'s `renderCurrentView` directly if this drifts again.)
