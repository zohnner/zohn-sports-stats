...
**NBA and NHL remain preview-only** — do not propose NBA/NHL feature work unprompted. **NCAA Football is a live surface (D-042, 2026-07-06):** Scores (offseason-aware), Rankings (AP/Coaches/CFP), conference-grouped Standings + Teams, and the home sport-picker band all shipped. Player leaders/detail were deferred (CFB player data too sparse). **A live game viewer Phase 1 skeleton shipped (D-118, 2026-08-22):** `js/ncaafLiveGame.js`, score header + venue/broadcast info only — the six-tab build (Play-by-Play/Box Score/Analytics), live situation, and win probability are named-and-deferred pending a live check against a real in-progress game (2026 season had not started as of D-118). NCAAF feature work is in scope.
...
| `js/ncaafLiveGame.js` | NCAAF live game viewer (D-118, 2026-08-22) — Phase 1 skeleton only: `showNCAAFGame(eventId)` reads `/api/ncaaf?path=/summary` (allowlisted since D-042, unused until now). Score header + venue/broadcast info chips, no CSS of its own (reuses `.nlg-*`/`.player-bio-item`/`.nfl-offseason`). Six-tab build, live situation/drives/plays rendering, and win probability are explicitly deferred — unverified against a real in-progress NCAAF game (2026 season had not started as of D-118; earliest game Aug 29). Do not build those tabs without a live check the way D-105/D-106 checked NFL |
...
- **Sport landings (D-045):** `functions/{mlb,nfl,ncaaf}/index.js` → `/mlb` `/nfl` `/ncaaf`; each sets `__SS_ROUTE={sport}-home` → the clean `_renderSportLanding(sport)` view in `js/app.js` (one hero + seasonal line + 4 entry cards). `SPORTS_META.defaultView` is `{sport}-home`, so entering a sport lands on its landing page (NFL's old `loadNFLHome` is kept but bypassed). **Gap found 2026-08-15 (DECISIONS.md D-102):** NCAAB and WNBA have none of this — no `functions/ncaab/`, no `functions/wnba/` directory at all (only their `/api/ncaab*`/`/api/wnba*` data-proxy Functions exist, which serve the SPA's own fetches, not crawlable content), no `/ncaab*`/`/wnba*` entries in `_routes.json`, and `tools/gen-sitemap.cjs` has zero references to either sport. Both are live surfaces (D-052, D-092) with zero crawlable landing/team/player pages and zero sitemap presence. Axiom confirmed this is a direct clone of the already-proven `functions/ncaaf/*` pattern — no architectural gate, just not yet built. See ISSUES.md, "SEO: NCAAB and WNBA have no edge-rendered path URLs" (2026-08-15).
...
- `sitemap.xml` lists the path URLs; regenerate it from live data with `node tools/gen-sitemap.cjs` (owner/CI-run — needs network; landings + MLB/NFL/NCAAF teams + players). Hash routes should canonicalize to their path URL. Content templates (player/team) now exist for **MLB, NFL and NCAAF** (`functions/{mlb,nfl,ncaaf}/{team,player}/...`).
