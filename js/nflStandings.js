...
// ── Power Rankings ──────────────────────────────────────────────────────
// Opponent-adjusted SRS (js/powerRankings.js) replacing the old win%/point-
// diff/streak composite -- that system had no strength-of-schedule
// adjustment at all. fetchNFLPowerTeamMeta feeds _PWR_SPORTS.nfl.fetchTeamMeta
// (wired in js/powerRankings.js, which loads after this file).
async function fetchNFLPowerTeamMeta() {
    ...
    rows.forEach(t => {
        meta[t.abbr] = {
            name: t.shortName,
            logo: t.logo,
            color: (typeof getNFLTeamColor === 'function' && getNFLTeamColor(t.abbr)) || null,
            record: `${t.wins}-${t.losses}${t.ties ? '-' + t.ties : ''}`,
            onClick: _nstdNav(t.abbr),
        };
    });
    return meta;
}
...
async function loadNFLPowerRankings() {
    ...
    if (typeof _prShow === 'function') await _prShow('nfl');
}
...
if (typeof window !== 'undefined') {
    ...
    window.fetchNFLPowerTeamMeta = fetchNFLPowerTeamMeta;
}
