...
const StatGlossary = (() => {
    ...
    const MLB = {
        ...
        GS:    'Games Started — number of games in which the pitcher was the starter',
        CG:    'Complete Games — games in which the starting pitcher pitched all innings',
        ...
        'QS%': 'Quality Start % — share of starts that qualify as quality starts',
        SVHD:  'Saves + Holds — combined relievers metric counting both saves and holds',
        ...
        GIDP:  'Grounded Into Double Play — times a batter grounded into a double play',
        RC:    'Runs Created — Bill James formula estimating offensive contribution: (H + BB) × TB ÷ (AB + BB)',
        'SB%': 'Stolen Base % — share of steal attempts that succeed: SB ÷ (SB + CS) × 100',
        'H/9': 'Hits per 9 Innings — hits allowed per 9 innings pitched',
        'HR/9':'Home Runs per 9 Innings — home runs allowed per 9 innings pitched',
        'K/BB':'Strikeout-to-Walk Ratio — strikeouts divided by walks; measures command + dominance',
        'LOB%':'Left on Base % — share of baserunners that do not score. Formula: (H + BB + HBP − R) ÷ (H + BB + HBP − 1.4 × HR). Higher = more strand ability.',
        wOBA:  'Weighted On-Base Average — weights each way of reaching base by its actual run value. Better than OBP at measuring offensive contribution.',
        'BB/K': 'Walk-to-Strikeout Ratio — walks divided by strikeouts; higher = better plate discipline',
        'AB/HR':'At-Bats per Home Run — how many at-bats it takes to hit one home run; lower = more power',
        'GB/FB':'Ground Ball to Fly Ball Ratio — ground outs divided by air outs',
        'HH%': 'Hard Hit % — share of batted balls with exit velocity ≥ 95 mph (Statcast)',
        'SS%': 'Sweet Spot % — share of batted balls with launch angle between 8° and 32° (optimal contact zone)',
        'Whiff%': 'Whiff % — share of swings that miss; higher = more swing-and-miss stuff',
        'CSW%': 'Called Strike + Whiff % — share of pitches that result in a called strike or a swinging strike; composite command + stuff metric',
        'EV Allowed': 'Average exit velocity allowed — lower is better for pitchers',
