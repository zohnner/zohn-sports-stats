...
- **`worker/`** — Cloudflare Worker for the BDL proxy (P1-006 fix target), plus `worker/wrangler-auth-purge.toml` (D-031) — a cron Worker that purges expired sessions/`audit_log` rows daily, sharing the same `USER_DB` D1 binding as the Pages Functions.
