// Pages Function: /nba/standings — crawlable, prerendered NBA conference +
// division standings page. Promotes the existing hash-only nba-standings
// view (js/nba.js's displayNBAStandings) to a real path URL. Clone of
// functions/wnba/standings.js — the same recursive collector, but NBA's real
// tree is conference > division > entries (2 conferences x 3 divisions
// each, confirmed live 2026-09-14), so this naturally yields 6 sections
// instead of WNBA's 2. Same HTML for humans and bots; fail-safe.
...
export async function onRequest(context) {
    ...
    try {
        ...
        const r = await fetch(new URL('/api/nbastandings', request.url), { cf: { cacheTtl: 1800, cacheEverything: true } });
        ...
        const canonical = 'https://sportstrata.cc/nba/standings';
        const title = `NBA Standings — Eastern & Western Conference | SportStrata`;
        const desc  = `NBA standings by conference and division — win-loss records, updated throughout the season. Free, no login, no ads.`;

        const confItems = sections.map(s => `<li>${esc(s.name)}</li>`).join('');
        const jsonld = JSON.stringify({
            '@context': 'https://schema.org', '@type': 'ItemList',
            name: 'NBA Conference & Division Standings', url: canonical,
        ...
        const snapshot =
            `<section class="ss-prerender"><h1>NBA Standings</h1>` +
            `<p>Eastern and Western Conference standings by division, with win-loss records, updated throughout the season on SportStrata — free, no login, no ads.</p>` +
        ...
        html = html
            ...
            .replace('</head>', `<script type="application/ld+json">${jsonld.replace(/</g, "\\u003c")}</script><script>window.__SS_ROUTE=${JSON.stringify('nba-standings')};</script></head>`)
        ...
        catch (_) { return Response.redirect('https://sportstrata.cc/#nba-standings', 302); }
    }
}
