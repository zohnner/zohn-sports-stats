// Pages Function: /nba/player/:id(/:slug) — crawlable, prerendered NBA
// player page. Clone of functions/wnba/player/[id]/[[slug]].js: NBA is
// ESPN-native end to end, so this fetches /api/nbaathlete directly, no
// name-match bridge needed. SPA shell + Person JSON-LD + snapshot +
// __SS_ROUTE=nba-player-{id}, matching js/nba.js's showNBAPlayer/
// displayNBAPlayerDetail route naming exactly.
//
// No team-page link (unlike the NFL template) — there is no NBA
// team-detail route yet, confirmed against js/nba.js before writing this.
...
export async function onRequest(context) {
    ...
    try {
        ...
        const r = await fetch(new URL(`/api/nbaathlete?id=${id}`, request.url), { cf: { cacheTtl: 21600, cacheEverything: true } });
        ...
        const canonical = `https://sportstrata.cc/nba/player/${id}/${slug}`;
        const route = 'nba-player-' + id;
        const title = `${name} — NBA Stats & Player Profile | SportStrata`;
        const desc  = `${name}${pos ? ', ' + pos : ''}${team ? ' · ' + team : ''} — NBA season stats and player profile. Free, no login.`;
        ...
        const snapshot =
            ...
            `<p>${bioLine}</p>` +
            `<p>${esc(name)} NBA season stats and player profile on SportStrata — free, no login, no ads.</p>` +
            statsHtml +
            `<p><a href="/nba">NBA Home</a> · <a href="/nba/leaders">NBA Stat Leaders</a></p></section>`;
