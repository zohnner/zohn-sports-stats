// Pages Function: /nba/leaders — crawlable, prerendered NBA stat leaders page.
// Clone of functions/wnba/leaders.js. Sources the already-curated /api/nbastats
...
// NBA team-detail route yet (js/nba.js's _renderNBAView has only a teams
// grid, no per-team view; confirmed before writing this). Each leader DOES
// link to /nba/player/:id/:slug since that page (and its underlying
// nba-player-{id} SPA view) is real.
...
export async function onRequest(context) {
    ...
    try {
        ...
        const r = await fetch(new URL('/api/nbastats', request.url), { cf: { cacheTtl: 900, cacheEverything: true } });
        ...
        const sections = categories.map(c => {
            ...
            const items = leaders.map(l => {
                const nameHtml = l.id ? `<a href="/nba/player/${esc(l.id)}/${esc(slug(l.name))}">${esc(l.name)}</a>` : esc(l.name);
        ...
        const canonical = 'https://sportstrata.cc/nba/leaders';
        const title = `NBA Stat Leaders ${season} — Points, Rebounds, Assists & More | SportStrata`;
        const desc  = `${season} NBA statistical leaders — points, rebounds, assists, steals and blocks per game, plus shooting percentages. Full leaderboards for every category. Free, no login, no ads.`;

        const headline = sections.find(s => s.c.key === 'pointsPerGame') || sections[0];
        const jsonld = JSON.stringify({
            '@context': 'https://schema.org', '@type': 'ItemList',
            name: `NBA ${headline.c.label} Leaders ${season}`, url: canonical,
        ...
        const snapshot =
            `<section class="ss-prerender"><h1>NBA Stat Leaders — ${esc(String(season))}</h1>` +
            `<p>Current ${esc(String(season))} NBA leaders across scoring, rebounding, playmaking and defensive categories. Full leaderboards for every stat on SportStrata — free, no login, no ads.</p>` +
            `<p><a href="/nba">NBA Home</a></p>` +
        ...
        html = html
            ...
            .replace('</head>', `<script type="application/ld+json">${jsonld.replace(/</g, "\\u003c")}</script><script>window.__SS_ROUTE=${JSON.stringify('nba-leaders')};</script></head>`)
        ...
        catch (_) { return Response.redirect('https://sportstrata.cc/#nba-leaders', 302); }
    }
}
