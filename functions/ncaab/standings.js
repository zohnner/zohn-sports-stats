// Pages Function: /ncaab/standings — crawlable, prerendered NCAA men's college
// basketball conference standings page. Promotes the existing hash-only
// ncaab-standings view (js/ncaab.js's displayNCAABStandings) to a real path
// URL. Clone of functions/ncaaf/standings.js — same recursive collector, same
// upstream shape (data.children = conferences, each with either
// standings.entries directly or children = divisions with entries; D-I
// basketball conferences are mostly flat per /api/ncaabstandings's own
// comment, but the collector handles either shape without special-casing).
// Same HTML for humans and bots; fail-safe. Self-fetches /api/ncaabstandings
// (same-origin) so season-default logic lives in one place, not duplicated here.
...
export async function onRequest(context) {
    ...
    try {
        ...
        const r = await fetch(new URL('/api/ncaabstandings', request.url), { cf: { cacheTtl: 1800, cacheEverything: true } });
        ...
        const canonical = 'https://sportstrata.cc/ncaab/standings';
        const title = `College Basketball Standings — Conference Records | SportStrata`;
        const desc  = `NCAA men's college basketball standings by conference — win-loss records for every Division I conference, updated throughout the season. Free, no login, no ads.`;

        const confItems = sections.map(s => `<li>${esc(s.name)}</li>`).join('');
        const jsonld = JSON.stringify({
            '@context': 'https://schema.org', '@type': 'ItemList',
            name: 'College Basketball Conference Standings', url: canonical,
        ...
        const snapshot =
            `<section class="ss-prerender"><h1>College Basketball Standings</h1>` +
        ...
        html = html
            ...
            .replace('</head>', `<script type="application/ld+json">${jsonld.replace(/</g, "\\u003c")}</script><script>window.__SS_ROUTE=${JSON.stringify('ncaab-standings')};</script></head>`)
        ...
        catch (_) { return Response.redirect('https://sportstrata.cc/#ncaab-standings', 302); }
    }
}
