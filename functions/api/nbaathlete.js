/**
 * Pages Function: /api/nbaathlete
 * Per-player NBA bio + season stat line from ESPN's core API.
 * Clone of functions/api/wnbaathlete.js — NBA is ESPN-native end to end, so
 * we already have the ESPN athlete id (from /api/nbastats leaders) and fetch
 * that athlete directly — no name-match bridge needed. Live-verified
 * 2026-09-14: NBA's per-athlete statistics categories/field names
 * (avgPoints/fieldGoalPct/avgRebounds/avgAssists/etc.) are identical to
 * WNBA's, so the same GROUPS config applies unchanged.
 *
 * Usage: /api/nbaathlete?id=3945274&season=2026
...
// NBA labels a season by its END year — see js/nba.js's NBA_LAST_SEASON for the same split.
...
export async function onRequest(context) {
    ...
    // wnbaathlete.js: only falls back to last season when `season` is the routine
    // computed default, never overrides an explicit ?season= request. Deliberately
    // does NOT touch bio/team above (roster-level resource populates before games
    // do, so the *current* team should show even if we fall back to last season's stats).
