/**
 * Pages Function: /api/ncaab
 * Same-origin server-side proxy for the ESPN men's college basketball public API.
 * Clones functions/api/ncaaf.js (D-042 pattern, D-052 for NCAAB) — fixes ESPN
 * browser CORS, keeps the frontend same-origin (no new connect-src host — ESPN
 * is already allowlisted).
 * No API keys, no D1 binding. Rate-limited by functions/api/_middleware.js.
 *
 * Usage: /api/ncaab?path=/scoreboard  (extra query params are forwarded)
 * Only an allowlisted set of ESPN CBB paths is permitted (no open proxy).
 */
// site.api.espn.com -> site.web.api.espn.com host swap — same fix as nfl.js/
// ncaaf.js (site.api.espn.com is Cloudflare-egress-blocked by ESPN's Akamai
// WAF; site.web.api.espn.com serves the identical /apis/site/v2/... shape).
const ESPN_NCAAB = 'https://site.web.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball';
...
function ttlFor(path) {
    ...
    if (path.startsWith('/rankings'))   return 1800;  // 30 min (AP / Coaches polls)
...
export async function onRequest(context) {
    ...
    const target = new URL(ESPN_NCAAB + path);
