/**
 * Pages Function: /api/nba
 * Same-origin server-side proxy for the ESPN NBA public API.
 * Clones functions/api/wnba.js (D-042/D-052/D-092 pattern, D-161 for NBA's
 * revival as a live 7th sport) — fixes ESPN browser CORS, keeps the frontend
 * same-origin (no new connect-src host — ESPN is already allowlisted).
 ...
 * blocks /stats and /season_averages (401), which is why NBA sat preview-only
 * since before the SPORTS registry existed (see DECISIONS.md D-052/D-161).
 *
 * Usage: /api/nba?path=/scoreboard  (extra query params are forwarded)
 * Only an allowlisted set of ESPN NBA paths is permitted (no open proxy).
 */
// site.api.espn.com -> site.web.api.espn.com host swap — same fix as nfl.js/
// ncaaf.js/ncaab.js/wnba.js (site.api.espn.com is Cloudflare-egress-blocked by
// ESPN's Akamai WAF; site.web.api.espn.com serves the identical
// /apis/site/v2/... shape).
const ESPN_NBA = 'https://site.web.api.espn.com/apis/site/v2/sports/basketball/nba';
...
export async function onRequest(context) {
    ...
    const target = new URL(ESPN_NBA + path);
